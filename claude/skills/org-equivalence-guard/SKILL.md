...
hooks:
  Stop:
    - hooks:
        - type: command
          command: bash $HOME/.claude/skills/equivalence-guard/scripts/completion_check.sh
          timeout: 20
---
...
3. 运行比对引擎：`bash ~/.claude/skills/equivalence-guard/scripts/api-equiv-diff.sh "$WORK_DIR"`。
