...
hooks:
  Stop:
    - hooks:
        - type: command
          command: bash $HOME/.claude/skills/scan/scripts/completion_check.sh
          timeout: 15
---
...
```bash
bash ~/.claude/skills/scan/scripts/project-stats.sh [项目路径]
bash ~/.claude/skills/scan/scripts/complexity-scan.sh [项目路径]
bash ~/.claude/skills/scan/scripts/dependency-stats.sh [项目路径]
bash ~/.claude/skills/scan/scripts/dir-tree.sh [项目路径]
```
...
| Agent 5 | Skills 质量 | SKILL.md 结构/角色/约束/Token效率/引用有效性 | 无 .claude/skills/ 目录 |
