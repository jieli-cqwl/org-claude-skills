...
3. NO repair without `baseline_ref` 和 `stash_ref` capture.
...
2. 建立 baseline：记录 `baseline_ref=$(git rev-parse HEAD)`；working tree dirty 或有 staged changes 时执行 `git stash push -m "review-fix-baseline-$(date +%s)"` 并记录 `stash_ref`（一次 stash 保护全部改动，不使用 `--keep-index`——双 stash 策略无法安全还原）；clean tree 且无 staged changes 时 `stash_ref = null`。
3. 执行评审引擎：
...
8. 异常矩阵统一 fail-close：
   ...
   - 用户中止或异常终止：保留现场，不执行恢复；报告 `baseline_ref`、`stash_ref`、当前轮次和已完成 commit，并提示 `git stash pop` 或 `git reset --soft <baseline_ref>`。
9. 退出恢复：
   - 正常终止、达到最大轮次或不收敛时，保留已生成的修复 commit。
   - `stash_ref != null` 时执行 `git stash pop` 恢复 working tree。冲突时只报告冲突文件和 `git stash show` 结果，不自动解决。注意：原 staged 状态恢复为 unstaged（git stash 固有行为），在报告中提示用户。
