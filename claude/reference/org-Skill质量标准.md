...
| `agents/openai.yaml` 存在（仅 Codex 侧） | 是 | Codex UI 展示所需，由统一仓库 `codex/skills/*/agents/openai.yaml` 提供，安装时覆盖到 `~/.codex/skills/` |
...
> `openai.yaml` 由统一仓库维护（`codex/skills/org-{name}/agents/openai.yaml`），通过 `install.sh` 安装到 Codex 侧（`~/.codex/skills/{name}/agents/openai.yaml`）。如需批量刷新可运行 `tools/install/generate-all-openai-yaml.sh`。
