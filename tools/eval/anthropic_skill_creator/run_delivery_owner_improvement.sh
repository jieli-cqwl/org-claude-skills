#!/usr/bin/env bash
# File responsibility: command-line entrypoint for the delivery-owner pilot of
# the Anthropic skill-creator local adapter.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
ADAPTER_DIR="$ROOT/tools/eval/anthropic_skill_creator"
CONFIG="$ADAPTER_DIR/configs/delivery-owner.json"
OUTPUT_DIR=""
MODE="full"
MODEL=""
JUDGE_MODEL=""
...
usage() {
  cat <<'USAGE'
Usage:
  bash tools/eval/anthropic_skill_creator/run_delivery_owner_improvement.sh [options]
