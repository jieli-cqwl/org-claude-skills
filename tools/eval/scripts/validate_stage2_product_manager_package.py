...
from validate_stage2_product_manager_phase_fields import check_phase_prd_fields  # noqa: E402
...
def check_package_envelope(package: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        package.get("input_origin")
        != "stage-2-product-director-confirmed-brief-package"
    ):
        add_failure(
            failures,
            "input_origin",
        ...
        for field in DIRECTOR_LOCK_FIELDS.get(artifact_type, ()):
            if current.get(field) != baseline.get(field):
                add_failure(
                    failures,
                    f"{key}.{field}",
                    "Director-owned field drift from confirmed brief",
                )
        if current.get("director_confirmation") != baseline.get(
            "director_confirmation"
        ):
            add_failure(
                failures,
                f"{key}.director_confirmation",
                "must preserve confirmed Director lock",
            )
...
def validate(package: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "stage2_readiness": "product_manager_prd_ready_for_design"
        if ready
        else "blocked",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Stage 2 product-manager package JSON.",
    )
