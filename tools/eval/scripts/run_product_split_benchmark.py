...
from product_split_benchmark_review import compare_outputs, generate_review
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Run the product split best-practice benchmark"
    )
    ...
    parser.add_argument(
        "--eval-ids", default=None, help="Comma-separated eval ids to run, e.g. 1,4"
    )
    ...
    if args.eval_ids:
        selected = {
            int(part.strip()) for part in args.eval_ids.split(",") if part.strip()
        }
        ...
        for config in CONFIGS:
            ...
            for run_number in range(1, args.runs_per_config + 1):
                ...
                run_executor(
                    eval_item,
                    config,
                    workspaces[config.bench_name],
                    run_dir,
                    args.model,
                    args.timeout_sec,
                )
                run_dirs.append(run_dir)
            eval_item.setdefault("representative_runs", {})[config.bench_name] = str(
                median_representative_run(run_dirs)
            )
    ...
    for eval_item in eval_items:
        ...
        response_by_config = {
            "with_skill": Path(
                eval_item["representative_runs"]["with_skill"]
            ).read_text(),
            "without_skill": Path(
                eval_item["representative_runs"]["without_skill"]
            ).read_text(),
        }
