...
def run_case(
    ...
    if grading_path.is_file():
        ...
        if not isinstance(grading, dict):
            raise ValueError(
                f"{skill_name}/{case['id']}: invalid existing grading output"
            )
        if (
            "infrastructure_failure" not in grading
            and grading.get("summary", {}).get("graded") is not False
        ):
            return summarize_grading(skill_name, case, run_dir, grading, args.run_mode)
        if not response_path.is_file():
            raise RuntimeError(
                f"{skill_name}/{case['id']}: previous infrastructure failure has no reusable response"
            )
    if not response_path.is_file():
...
def write_summary(output_dir: Path, runs: list[dict]) -> dict:
    ...
    optimization_findings = [
        finding for run in runs for finding in run.get("optimization_findings", [])
    ]
    summary = {
        "runs": runs,
        "summary": {
    ...
    for run in runs:
        if run.get("infra_failure"):
            markdown.append(
                f"- {run['skill_name']} / {run['eval_id']}: infra failure (ungraded)"
            )
            markdown.append(f"  - infra failure: {run['infra_failure']}")
        else:
            markdown.append(
                f"- {run['skill_name']} / {run['eval_id']}: {run['passed']}/{run['total']} passed"
            )
        ...
        try:
            ...
            failure_cases = cases or [
                {
                    ...
                    "expectations": [
                        "The eval runner can load cases and prepare the skill workspace before grading."
                    ],
                }
            ]
            for case in failure_cases:
                for run_number in range(1, args.runs_per_eval + 1):
                    ...
                    runs.append(
                        record_infra_failure(skill_name, case, run_dir, exc, args)
                    )
            continue
        for case in cases:
            for run_number in range(1, args.runs_per_eval + 1):
                ...
                try:
                    workspace = prepare_workspace(
                        skill_name, args.output_dir, args.run_mode
                    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--skills",
        required=True,
        help="Comma-separated skill names, e.g. product-director,product-manager",
    )
    parser.add_argument(
        "--eval-ids", default=None, help="Comma-separated eval ids to run"
    )
    parser.add_argument("--runs-per-eval", type=int, default=1)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "tools" / "eval" / "results" / "standard-chain-local",
    )
    ...
    parser.add_argument(
        "--hide-expectations",
        dest="hide_expectations",
        action="store_true",
        help="Do not show expectations to the executor; judge still uses them for grading.",
    )
    ...
    if (
        summary["summary"]["failed_expectations"]
        or summary["summary"]["infra_failures"]
