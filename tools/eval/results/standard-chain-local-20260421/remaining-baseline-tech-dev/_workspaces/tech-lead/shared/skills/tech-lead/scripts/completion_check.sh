...
extract_plan_fixed_gate_stages() {
    local plan_file="$1"
    local gate_section

    gate_section=$(extract_markdown_section "$plan_file" "## Phase 3 固定完整门禁")

    printf '%s\n' "$gate_section" | grep -oE 'REVIEW_[A-Z]+|QA_[A-Z]+' | while IFS= read -r stage; do
        [ -n "$stage" ] || continue
...
extract_plan_waived_stages() {
    ...
    phase3_section=$(extract_markdown_section "$plan_file" "## Phase 3 固定完整门禁")
...
check_phase3_fixed_gate() {
    local plan_file="$1"
    local gate_stages required_reviews required_qas required_stages missing_stages unexpected_stages stage waived_stages

    gate_stages=$(extract_plan_fixed_gate_stages "$plan_file")
    if [ -z "$gate_stages" ]; then
        add_failure "T7c: plan.md 的「Phase 3 固定完整门禁」缺少可解析门禁阶段"
        return 0
    fi

    required_reviews=$(phase3_required_review_stages 2>/dev/null || true)
    required_qas=$(phase3_required_qa_stages 2>/dev/null || true)
    ...
    if [ -n "$missing_stages" ]; then
        add_failure "T7c: plan.md 的 Phase 3 固定完整门禁缺少阶段：${missing_stages}"
    fi
    if [ -n "$unexpected_stages" ]; then
        add_failure "T7c: plan.md 的 Phase 3 固定完整门禁多出阶段：${unexpected_stages}"
    fi
...
# T7b: Phase 3 fixed full gate must be parseable.
if ! has_any_fixed_section "$PLAN_FILE" "## Phase 3 固定完整门禁"; then
    add_failure "T7b: plan.md 缺少 Phase 3 固定完整门禁章节"
fi

check_phase3_fixed_gate "$PLAN_FILE"
