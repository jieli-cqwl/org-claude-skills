...
probe_default_surface() {
  ...
  if [ ! -f "$CODEX_HOME/skills/brainstorming/agents/openai.yaml" ] \
    ...
    || [ -f "$CODEX_HOME/skills/product-manager/agents/openai.yaml" ] \
    || [ -f "$CODEX_HOME/skills/product/agents/openai.yaml" ]; then
    fail_check "small-chain 自动暴露面不符合预期（brainstorming 应自动暴露，using-superpowers/product-director/product-manager/product 应为 manual-only）"
