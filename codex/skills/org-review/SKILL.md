...
> 本 skill 为纯审查（fix_mode=none），仅引用内层审查递增协议（`references/review-iteration-protocol.md`），不引用外层修复循环协议（`reference/review-fix-loop-protocol.md`）。审完即止，不执行修复循环。

- 收集变更文件、变更统计、最近提交，确认本轮审查边界。
- 读取 `references/review-iteration-protocol.md` 作为轮次与收敛准绳。
