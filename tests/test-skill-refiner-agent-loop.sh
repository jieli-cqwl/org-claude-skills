...
assert_present '共创草案必须先给推荐草案、依据和需要用户确认的最小事实；不把空白问题直接抛给用户' "$SKILL"
assert_present 'SR-S2 对用户输出必须使用“入口基线确认卡”，用人话标签呈现为：场景、约束、想看到的变化、观察到的不适、要保留的能力、候选切入点、承载、待确认' "$SKILL"
assert_present '`expected_outcome_signal` 的用户侧名称是“想看到的变化”，只表示用户希望优化后出现的可观察变化线索；不是最终成功标准、验收口径或验证命令' "$SKILL"
assert_present 'SR-S2 不得把 `co_created_baseline`、`expected_outcome_signal` 等 schema key 作为用户侧主标题；schema key 只写入台账或结果 JSON' "$SKILL"
assert_present 'SR-S2 草案不得使用“当前判断”“最佳实践目标”“候选策略”“验证方式”作为本步标题；不得用“因为/所以”把观察痛点写成已定位根因' "$SKILL"
assert_present 'SR-S3、SR-R1~SR-R10 和 SR-F1 的草案可使用当前判断、最佳实践目标、最佳实践来源、适用/不适用条件、保留能力、问题证据、候选策略、验证方式和需要用户确认的问题' "$SKILL"
assert_present 'SR-F1 前只允许写入或更新本轮 `refinement-ledger.json`；目标 Skill、测试、runtime、引用和文档入口仍不得修改' "$SKILL"
assert_present '每轮继续打磨前读取 `references/problem-framing.md`，只提取问题定义卡反问口径，并用它反问：为什么这是下一刀、对应哪个环节和质量维度、消费者是谁、如何验证；答不清时只登记候选缺口，不改文件' "$SKILL"
...
assert_present '"anchor": "先和用户共创入口基线，对用户用入口基线确认卡呈现场景、约束、想看到的变化、观察到的不适、要保留的能力、候选切入点、承载和待确认；内部再映射到 co_created_baseline 字段；SR-S2 不使用当前判断、最佳实践目标、候选策略、验证方式或 schema key 作为用户侧标题；不得在 SR-S2 产出根因、最终成功标准或最终操作判断；再改文件"' "$ROOT/shared/skills/skill-refiner/evals/evals.json"
...
assert_present '主导共创产品 Skill 的入口基线，对用户用入口基线确认卡确认场景、约束、想看到的变化、观察到的不适、要保留的能力和候选切入点，内部再映射到 co_created_baseline 字段' "$ROOT/shared/skills/skill-refiner/test-prompts.json"
