...
assert_absent '每个 SR-R 环节都必须记录 `best_practice_sources`、`source_conflicts`、`applicability` 和 `non_applicability`' "$SKILL"
assert_absent '来源类型从官方、GitHub、社区、本仓库实践和用户上下文中按需选择，不能只在 Flow 环节调研最佳实践' "$SKILL"
assert_absent "$blocked_creator_route" "$SKILL"
assert_absent 'SR-S2 只确认入口事实、用户反馈和假设边界；不得把根因、最终成功标准、最终操作判断或环节策略写成已确认结论' "$SKILL"
assert_present '真实场景、业务约束、用户预期结果线索、已观察痛点、不可丢能力候选、本轮切入点候选、已定位承载和未确认缺口' "$SKILL"
assert_absent '你把用户反馈、质量标准、rubric、旧测试和运行入口转成可冻结、可执行、可验证的精修策略' "$SKILL"
assert_absent '用户修正优先于 rubric 建议；rubric 用于给出专业判断依据，不替用户确认目标' "$SKILL"
...
assert_present '读取：本轮改动、`references/noise-taxonomy.md` 和相关测试断言，用于残留噪音扫描；验证失败时只读取本轮改动相关文件' "$SKILL"
...
assert_absent '禁止事项：SR-S2 只确认入口事实、用户反馈和假设边界' "$SKILL"
...
assert_present '做什么：读取最新台账，沉淀真实办事顺序、阶段闸门、失败分支和目标闭合条件' "$SKILL"
assert_present '做什么：读取最新台账，沉淀默认产物、消费者、人工摘要边界和机器结果事实源' "$SKILL"
assert_present '做什么：读取最新台账，沉淀主 SOP、reference、script、schema、template、eval 和 test 的职责分层' "$SKILL"
assert_present '做什么：读取最新台账，沉淀哪些判断必须外移到脚本、schema 或测试，以及执行入口和失败结果' "$SKILL"
assert_present '做什么：读取最新台账，沉淀何时触发、何时分流给相邻 Skill、创建/重写/拆分候选操作何时只能登记、何时必须后置到 SR-F1 裁决' "$SKILL"
...
assert_present '最终修改、迁移、删除、保留或新建的裁决只在 SR-F1 冻结' "$SKILL"
