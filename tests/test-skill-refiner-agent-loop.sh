...
assert_present 'SR-S2 草案必须按 `co_created_baseline` 字段逐项呈现：`real_scenario`、`business_constraint`、`expected_outcome_signal`、`observed_pain`、`protected_capability_candidate`、`entry_point_candidate`、`located_carrier`、`open_questions`' "$SKILL"
...
assert_present '"anchor": "先和用户共创入口基线，按 co_created_baseline 字段逐项补齐真实场景、业务约束、用户预期结果线索、已观察痛点、不可丢能力候选、本轮切入点候选、已定位承载和未确认缺口；SR-S2 不使用当前判断、最佳实践目标、候选策略或验证方式作为标题；不得在 SR-S2 产出根因、最终成功标准或最终操作判断；再改文件"' "$ROOT/shared/skills/skill-refiner/evals/evals.json"
...
assert_present '主导共创产品 Skill 的入口基线，按 co_created_baseline 字段逐项确认真实场景、业务约束、用户预期结果线索、已观察痛点、不可丢能力候选和本轮切入点候选' "$ROOT/shared/skills/skill-refiner/test-prompts.json"
