...
assert_present '使用 sub agent 分担脚本结果整理、只读采证和候选方案起草' "$DESIGN_SKILL"
assert_present '你的主上下文只保留决策所需事实' "$DESIGN_SKILL"
assert_present '你亲自复核 sub agent 结果' "$DESIGN_SKILL"
assert_present '所有设计裁决、用户确认、候选包写入、最终 `design\.json`、验证、投影验收和下游交接都由你负责' "$DESIGN_SKILL"
...
assert_present 'S8 候选设计包只写入 `\$TMPDIR`；S10 用户确认且验证通过前，不写 `\{phase_dir\}/design\.json`' "$DESIGN_SKILL"
...
assert_present '采证对象包含部署、配置中心、数据源或外部服务时，读取 `references/runtime-fact-capture\.md`，只提取只读命令边界和 `runtime_facts` 字段要求' "$DESIGN_SKILL"
assert_present 'S4 开始质量属性排序前，读取 `references/quality-attributes\.md`，只提取优先级、场景、目标指标和权衡字段' "$DESIGN_SKILL"
assert_present 'S5 处理每个关键决策前，读取 `references/decision-templates\.md`，只提取候选方案、取舍、失效条件和用户确认字段' "$DESIGN_SKILL"
...
assert_present 'S6 定义接口契约前，读取 `references/interface-spec\.md`，只提取 `input_params / output_params / error_codes / boundary_behaviors`' "$DESIGN_SKILL"
assert_present 'S7 处理技术风险、迁移风险或回滚触发条件时，读取 `references/risk-assessment\.md`，只提取风险回应、验证引用和回滚触发条件' "$DESIGN_SKILL"
assert_present 'S9 创建 reviewer 前，读取对应 reviewer prompt，只提取审查范围、digest 回显和报告格式' "$DESIGN_SKILL"
...
assert_present 'S10 抽样验收发现投影字段遗漏、ADR 约束不完整或需要修改 renderer 行为时，读取 `projections/design-template\.md` 或 `projections/adr-spec\.md`，只提取字段来源和决策引用规则' "$DESIGN_SKILL"
