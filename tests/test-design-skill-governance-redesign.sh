...
assert_design_preflight_manifest_contract() {
  jq -e '
    ...
    | .id == "preflight-check"
      ...
      and (.allowed_args | index("--arguments") != null)
      and (.allowed_args | index("--help") != null)
      and (.allowed_args | index("-h") != null)
      and .timeout_seconds == 15
...
assert_design_preflight_passes_ready_phase() {
  ...
  phase_arg_out="$tmp_dir/preflight-phase-arg.stdout"
  feature_arg_out="$tmp_dir/preflight-feature-arg.stdout"
  ...
  if ! bash "$DESIGN_PREFLIGHT" --arguments "$tmp_dir/docs/sample-feature/phase-1" >"$phase_arg_out" 2>"$stderr_file"; then
    cat "$phase_arg_out" >&2
    cat "$stderr_file" >&2
    rm -rf "$tmp_dir"
    fail "design preflight should accept phase path via --arguments"
  fi
  if ! bash "$DESIGN_PREFLIGHT" --arguments "$tmp_dir/docs/sample-feature" >"$feature_arg_out" 2>"$stderr_file"; then
    cat "$feature_arg_out" >&2
    cat "$stderr_file" >&2
    rm -rf "$tmp_dir"
    fail "design preflight should accept feature path via --arguments"
  fi
  jq -e --arg phase_dir "$(jq -r '.phase_dir' "$stdout_file")" '
    .status == "PASS"
  ...
  ' "$phase_arg_out" >/dev/null || {
    cat "$phase_arg_out" >&2
    rm -rf "$tmp_dir"
    fail "design preflight --arguments phase path must return stable canonical input paths"
  }
  jq -e --arg phase_dir "$(jq -r '.phase_dir' "$stdout_file")" '
    .status == "PASS"
    and .phase_dir == $phase_dir
  ...
  ' "$feature_arg_out" >/dev/null || {
    cat "$feature_arg_out" >&2
    rm -rf "$tmp_dir"
    fail "design preflight --arguments feature path must select the stable phase input paths"
  }
  rm -rf "$tmp_dir"
}
...
assert_present '^## 流程$' "$TEST_DESIGN_SKILL"
assert_present 'digraph test_design_flow' "$TEST_DESIGN_SKILL"
assert_absent '^## 流转图$|^## 流程细节$|```mermaid|graph TD' "$TEST_DESIGN_SKILL"
...
assert_present '效果增值' "$STANDARD"
assert_present '触发准确' "$STANDARD"
assert_present 'Professional Workflow' "$STANDARD"
assert_present 'Artifact Contract' "$STANDARD"
assert_present 'Verification Loop' "$STANDARD"
assert_absent '设计覆盖清单|Q1-Q9|Q1[[:space:]]*技术现状|Q9[[:space:]]*风险|semantic closure' "$DESIGN_SKILL"
assert_present '读取 template/schema，确认当前产物只能写入已定义字段' "$DESIGN_SKILL"
assert_present '只写入 template/schema 已定义字段' "$DESIGN_SKILL"
assert_absent '架构判断.*写入位置.*确定性验证|确定性验证.*写入位置.*架构判断' "$DESIGN_SKILL"
assert_present '^## 角色$' "$DESIGN_SKILL"
...
assert_present '负责所有设计裁决、用户确认、候选包写入、最终 `design\.json`、验证、下游交接，以及可选投影的抽样验收' "$DESIGN_SKILL"
...
assert_present '脚本返回的 `phase_dir`、`brief`、`phase_prd`、`units`、可选 `constitution` 和可选 `ledger`' "$DESIGN_SKILL"
...
assert_present '"S9 三视角评审" -> "S10 最终确认、写入与验证" \[label="PASS/WARN承接"\]' "$DESIGN_SKILL"
assert_present '"S10 最终确认、写入与验证" -> "S11 可选投影渲染" \[label="PASS 且需要人类视图/ADR"\]' "$DESIGN_SKILL"
assert_present '"S10 最终确认、写入与验证" -> "交给 /test-design" \[label="PASS 且无需投影"\]' "$DESIGN_SKILL"
assert_present '"S10 最终确认、写入与验证" -> "修正本轮设计" \[label="FAIL"\]' "$DESIGN_SKILL"
assert_present '"S11 可选投影渲染" -> "交给 /test-design" \[label="你抽样验收"\]' "$DESIGN_SKILL"
...
assert_present 'S4 开始质量属性排序前，把每个质量属性写成质量场景' "$DESIGN_SKILL"
assert_present '每轮只处理一个关键决策：先给事实锚点' "$DESIGN_SKILL"
assert_present '技术选型依赖最新外部事实且本地资料不足时，才使用 WebSearch，并在 `option_analysis` 记录来源' "$DESIGN_SKILL"
assert_present 'S6 定义接口契约时，每个接口在 `interfaces` 或 `interface_boundary` 写清 `input_params / output_params / error_codes / boundary_behaviors`' "$DESIGN_SKILL"
assert_present 'S7 处理技术风险、迁移风险或回滚触发条件时，在 `risk_response` 写清风险回应、验证引用和回滚触发条件' "$DESIGN_SKILL"
...
assert_present '脚本只写投影草稿和 manifest' "$DESIGN_SKILL"
assert_present 'render_projection\.py --design "\$PHASE_DIR/design\.json" --adr-dir "\$PHASE_DIR/adr"' "$DESIGN_SKILL"
assert_present '脚本只写 ADR 草稿' "$DESIGN_SKILL"
assert_present '抽样验收发现投影字段遗漏、ADR 约束不完整或需要修改 renderer 行为时，读取 `projections/design-template\.md` 或 `projections/adr-spec\.md`；定位问题时只使用字段来源和决策引用规则' "$DESIGN_SKILL"
...
assert_absent 'references/canonical-ref-cheatsheet\.md' "$DESIGN_SKILL"
assert_absent 'references/architecture-patterns\.md' "$DESIGN_SKILL"
assert_absent 'references/decision-templates\.md' "$DESIGN_SKILL"
assert_absent 'references/interface-spec\.md' "$DESIGN_SKILL"
assert_absent 'references/legacy-modernization\.md' "$DESIGN_SKILL"
assert_absent 'references/quality-attributes\.md' "$DESIGN_SKILL"
assert_absent 'references/risk-assessment\.md' "$DESIGN_SKILL"
assert_absent 'references/service-decomposition\.md' "$DESIGN_SKILL"
test ! -f "$ROOT/shared/skills/design/references/canonical-ref-cheatsheet.md" || fail "canonical-ref-cheatsheet.md must be removed"
test ! -f "$ROOT/shared/skills/design/references/architecture-patterns.md" || fail "architecture-patterns.md must be removed"
test ! -f "$ROOT/shared/skills/design/references/decision-templates.md" || fail "decision-templates.md must be removed"
test ! -f "$ROOT/shared/skills/design/references/interface-spec.md" || fail "interface-spec.md must be removed"
test ! -f "$ROOT/shared/skills/design/references/legacy-modernization.md" || fail "legacy-modernization.md must be removed"
test ! -f "$ROOT/shared/skills/design/references/quality-attributes.md" || fail "quality-attributes.md must be removed"
test ! -f "$ROOT/shared/skills/design/references/risk-assessment.md" || fail "risk-assessment.md must be removed"
test ! -f "$ROOT/shared/skills/design/references/service-decomposition.md" || fail "service-decomposition.md must be removed"
...
assert_present '已验证 `design\.json`' "$ROOT/shared/skills/design/projections/design-template.md"
...
assert_present 'S11 可选投影只在 S10 所有 validator 通过后按需运行 render_projection\.py' "$DESIGN_EVALS"
...
for design_method_reference in \
  "$ROOT/shared/skills/design/references/runtime-fact-capture.md"; do
...
assert_present 'test_refs' "$VERIFY_SKILL"
assert_present 'assertion_target' "$VERIFY_SKILL"
assert_present 'evidence_expectation' "$VERIFY_SKILL"
