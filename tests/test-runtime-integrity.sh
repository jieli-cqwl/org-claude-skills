...
test -f "$CODEX_SKILLS_DIR/skill-creator/SKILL.md" || fail "Codex user runtime should install repository-managed skill-creator"
auto_policy skill-creator || fail "skill-creator should remain auto"
...
grep -Fq "$TMP_HOME/.codex/hooks/managed/codex_subagent_dispatch_guard.py" "$TMP_HOME/.codex/hooks.json" || fail "codex hooks should include subagent dispatch guard"
