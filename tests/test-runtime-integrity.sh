...
for skill in ai-cli-updater feishu-docs deep-research algorithmic-art brand-guidelines canvas-design darwin-skill ui-ux-pro-max doc-coauthoring docx internal-comms mcp-builder pdf pptx slack-gif-creator theme-factory web-artifacts-builder xlsx agent-browser; do
...
for runtime_dir in "$TMP_HOME/.claude" "$TMP_HOME/.codex"; do
  ...
  assert_runtime_absent '^## Runtime Contract$' "$runtime_dir/rules/代码规范.md"
  assert_runtime_present '本页 MUST 条款是代码规范真源' "$runtime_dir/rules/代码规范.md"
  assert_runtime_present 'reference 不能放宽 MUST' "$runtime_dir/rules/代码规范.md"
  ...
  assert_runtime_present '禁止把执行动作、中间步骤或工具结果当作目标达成' "$runtime_dir/rules/执行纪律.md"

  assert_runtime_absent '^## Runtime Contract$' "$runtime_dir/rules/文档管理.md"
  assert_runtime_absent '^## 边界$' "$runtime_dir/rules/文档管理.md"
  assert_runtime_absent 'agent 必须执行的文档判断' "$runtime_dir/rules/文档管理.md"
  assert_runtime_present '行为、规则、contracts、工具或测试任一变化影响同一约束时，相关文档必须同步更新' "$runtime_dir/rules/文档管理.md"
  assert_runtime_present "受管 feature/task 目录使用 \`--\` 分隔语义段" "$runtime_dir/rules/文档管理.md"
  assert_runtime_present '发现过时文档时，必须在本次变更内修正或归档' "$runtime_dir/rules/文档管理.md"
