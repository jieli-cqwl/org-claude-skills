...
jq '.design_stage_confirmations |= map(select(.stage_id != "design-synthesis"))' \
...
assert_present 'design_stage_confirmations|design-synthesis|required confirmation stage' "$MISSING_STAGE_WORKSPACE/hook.stderr" "missing stage hook stderr"
