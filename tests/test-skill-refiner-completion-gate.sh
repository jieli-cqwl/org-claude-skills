...
assert_present 'SR-S2 不使用当前判断、最佳实践目标、候选策略、验证方式或 schema key 作为用户侧标题' "$SKILL"
assert_present 'SR-R1~SR-R10 每个环节都有台账结论、目标形态、保留能力、问题证据、候选策略和验证方式' "$SKILL"
...
assert_present '来源冲突和适用/不适用条件' "$SKILL"
