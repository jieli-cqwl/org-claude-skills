...
assert_present 'SR-S2 对用户输出必须使用“入口基线确认卡”' "$SKILL"
assert_present '`expected_outcome_signal` 的用户侧名称是“想看到的变化”' "$SKILL"
assert_present 'schema key 只写入台账或结果 JSON' "$SKILL"
assert_present 'SR-S2 草案不得使用“当前判断”“最佳实践目标”“候选策略”“验证方式”作为本步标题' "$SKILL"
