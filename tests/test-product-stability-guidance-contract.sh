...
assert_present '每轮共创收口' "$CONVERSATION_GUIDE"
assert_present '最佳实践推荐 \+ 业务适配裁决|最佳实践推荐' "$CONVERSATION_GUIDE"
assert_present '不把最佳实践拆成多个同级选项交给用户选择' "$CONVERSATION_GUIDE"
assert_present '确认该推荐是否适配你的业务' "$CONVERSATION_GUIDE"
assert_present '对话节奏' "$CONVERSATION_GUIDE"
assert_present '第一性原理追问' "$CONVERSATION_GUIDE"
assert_present '警示信号' "$CONVERSATION_GUIDE"
assert_present 'Director 关键追问主题' "$CONVERSATION_GUIDE"
assert_present '深度路由' "$CONVERSATION_GUIDE"
assert_present '确认门前自检' "$CONVERSATION_GUIDE"
assert_absent 'D-S2~D-G1|D-S1 候选线索|全共创用于 D-S|草案修正用于 D-S|^## D-G1 前自检$|\| D-S[0-9]' "$CONVERSATION_GUIDE"
assert_absent '本文件只承载|跨环节复用的基础对话规则|^## 使用边界$|^## 裁决问题格式$|^## 主导共创$|^## 沟通风格$' "$CONVERSATION_GUIDE"
assert_absent '必要时给 2-3 个选项|推荐选项|请确认 / 选择 / 修正|多选题优先|用户只做选择|让用户选择最接近' "$CONVERSATION_GUIDE"
...
assert_present '每轮共创收口' "$CONVERSATION_GUIDE"
assert_present '提出裁决问题.*references/conversation-guide\.md|references/conversation-guide\.md.*一个共创收口动作' "$SKILL"
assert_present '不从该文件推导根问题、成功标准、范围、风险、Phase 规划或输出字段' "$SKILL"
assert_present '提问节奏、暂停点和确认门自检不得产生业务结论' "$SKILL"
...
assert_present '只能在总监确认门收到用户明确 `产品总监确认`' "$DIRECTOR_PHASE_GUIDE"
