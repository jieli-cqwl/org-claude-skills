...
assert_present '方案探索退出规则' "$CONVERSATION_GUIDE"
assert_present '用户输入处理（输入归一化）' "$CONVERSATION_GUIDE"
assert_present '推荐结论草案（推荐结论 \+ 推荐理由 \+ 未闭合假设）' "$CONVERSATION_GUIDE"
assert_present '业务事实支撑关键假设时，将推荐结论写入 checkpoint' "$CONVERSATION_GUIDE"
assert_present '回应包含已闭合上游事实的替换事实时，回到闭合该事实的步骤重新验证' "$CONVERSATION_GUIDE"
assert_present '用户回复业务事实后，用本节决定当前步骤推进、草案重写、上游事实重开或候选线索登记' "$CONVERSATION_GUIDE"
