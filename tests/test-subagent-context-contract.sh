...
assert_absent '^[[:space:]]*##[[:space:]]+静默降噪$' "$DIRECTOR_D_S2"
assert_absent 'Context Scan Agent' "$DIRECTOR_D_S2"
assert_absent 'Problem Hypothesis Agent' "$DIRECTOR_D_S2"
assert_absent 'sub agent' "$DIRECTOR_D_S2"
assert_absent 'subagent' "$DIRECTOR_D_S2"
assert_absent '子代理' "$DIRECTOR_D_S2"
assert_present '候选线索' "$DIRECTOR_D_S2"
assert_present '主 Agent 直接进入正常共创节奏' "$DIRECTOR_D_S2"
assert_present '必须回到用户追问' "$DIRECTOR_D_S2"
