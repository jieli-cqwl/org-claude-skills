...
DIRECTOR_EVIDENCE_MAP="$ROOT/shared/skills/product-director/references/evidence-map.md"
DIRECTOR_ROOT_PROBLEM="$ROOT/shared/skills/product-director/references/root-problem.md"
for director_ref in "$DIRECTOR_ROLE_MINDSET" "$DIRECTOR_EVIDENCE_MAP" "$DIRECTOR_ROOT_PROBLEM"; do
  test -f "$director_ref" || fail "missing director reference: ${director_ref#"$ROOT"/}"
  assert_absent 'Context Scan Agent' "$director_ref"
  assert_absent 'Problem Hypothesis Agent' "$director_ref"
  assert_absent 'sub agent' "$director_ref"
  assert_absent 'subagent' "$director_ref"
  assert_absent '子代理' "$director_ref"
done
assert_present '主导共创' "$DIRECTOR_ROLE_MINDSET"
assert_present '会改变根问题、目标、范围、风险、Phase 或冻结条件的冲突' "$DIRECTOR_EVIDENCE_MAP"
assert_present '方案线索 -> 受影响角色 -> 触发场景' "$DIRECTOR_ROOT_PROBLEM"
