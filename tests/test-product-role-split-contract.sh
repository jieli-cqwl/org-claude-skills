...
assert_present '验证关键业务假设.*references/conversation-guide\.md|references/conversation-guide\.md.*共创回合协议' "$PRODUCT_DIRECTOR_ROOT/SKILL.md"
...
assert_present '问题澄清共创形态' "$DIRECTOR_PROBLEM_GUIDE"
assert_present '推荐问题框架' "$DIRECTOR_PROBLEM_GUIDE"
assert_present '若 Y 不成立，给出替换事实' "$DIRECTOR_PROBLEM_GUIDE"
assert_present '默认推荐和触发条件' "$DIRECTOR_PROBLEM_GUIDE"
assert_absent '不得把方法论最佳实践拆成选项让用户判断' "$DIRECTOR_PROBLEM_GUIDE"
assert_present '共创回合协议' "$MANAGER_CONVERSATION_GUIDE"
assert_present '业务事实回应处理' "$MANAGER_CONVERSATION_GUIDE"
assert_present '模式差异' "$MANAGER_CONVERSATION_GUIDE"
assert_present '推荐结论、推荐理由和会改变结论的未闭合业务假设' "$MANAGER_CONVERSATION_GUIDE"
assert_present '回应中的业务事实支撑关键假设时，将 PM 推荐结论作为当前步骤结论写入 checkpoint' "$MANAGER_CONVERSATION_GUIDE"
assert_present '回应包含 Director 锁定字段、Phase 边界、范围或约束事实的替换事实时，回退 `/product-director`' "$MANAGER_CONVERSATION_GUIDE"
