...
assert_present '每轮对话节奏' "$MANAGER_CONVERSATION_GUIDE"
...
assert_present '收口确认' "$MANAGER_CONVERSATION_GUIDE"
assert_present '交付确认' "$MANAGER_CONVERSATION_GUIDE"
assert_present 'PM 推荐结论草案（推荐结论 \+ 推荐理由 \+ 未闭合假设）' "$MANAGER_CONVERSATION_GUIDE"
assert_present '业务事实支撑关键假设时，将 PM 推荐结论写入 checkpoint' "$MANAGER_CONVERSATION_GUIDE"
assert_present '回应包含 Director 锁定字段、Phase 边界、范围或约束事实的替换事实时，停止当前动作，报告冲突事实和建议入口；是否进入 `/product-director` 由用户裁决' "$MANAGER_CONVERSATION_GUIDE"
assert_present '不得写成“回退 / 回到 / 下一步执行 `/product-director`”' "$MANAGER_CONVERSATION_GUIDE"
...
assert_present '主导技术共创：先给推荐方案、备选方案和取舍理由' "$DESIGN_SKILL"
assert_absent 'product-manager/references/conversation-guide\.md|核心参考：.*product-manager' "$DESIGN_SKILL"
