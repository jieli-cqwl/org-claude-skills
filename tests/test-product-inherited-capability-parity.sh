...
assert_present 'references/d-s3-success-appetite\.md' "$DIRECTOR_SKILL"
assert_present 'references/d-s5-scope-constraints\.md' "$DIRECTOR_SKILL"
...
assert_present '^# D-S3 Success Criteria and Appetite$' "$DIRECTOR_D_S3"
assert_present '价值假设验证' "$DIRECTOR_D_S3"
assert_present '当前基线' "$DIRECTOR_D_S3"
assert_present '目标值或方向' "$DIRECTOR_D_S3"
assert_present '观测窗口' "$DIRECTOR_D_S3"
assert_present '数据来源' "$DIRECTOR_D_S3"
assert_present '^# D-S5 Scope, Constraints, and Rationale$' "$DIRECTOR_D_S5"
