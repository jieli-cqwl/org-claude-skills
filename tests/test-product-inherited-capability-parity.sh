...
assert_present '^## product-manager-review\.md 产物契约$' "$MANAGER_REVIEW_CONTRACT"
assert_present 'product-manager-review\.md 是 Manager 阶段的评审闭环证据文件' "$MANAGER_REVIEW_CONTRACT"
assert_present '写入 `product-manager-review\.md` 时使用以下收口规则，不要依赖 gate 去猜' "$MANAGER_REVIEW_CONTRACT"
