---
name: good-package
description: Use when auditing a Skill package for trigger, workflow, artifact, verification, lifecycle, and retain evidence quality.
allowed-tools: Read
---
...
- Stop when lifecycle evidence is required but `evals/lifecycle-review.json` is unreadable.
...
| check-contract | Evidence summary | Check trigger, workflow, artifact, verification, lifecycle, and retain gates | Finding list | user and tests | Findings map to standard dimensions | Stop on FAIL or return WARN/PASS | Checker JSON |
