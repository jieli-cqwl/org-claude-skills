...
| R03 | 未定义产物 | `product-manager-review.md` 被直接要求维护，但未先说明它是什么 | 评审状态改为写入 JSON 字段，人类投影视图不作为下游控制输入 | `test-product-context-signal-quality.sh` |
| R04 | 自述噪音 | `本契约定义`、`## 适用范围` 会复述外部引用关系 | reference 直接进入可执行规则 | `test-product-context-signal-quality.sh` |
| R05 | 入口元说明 | `SKILL.md 只保留...`、`真源` 是作者说明 | 入口只保留读取哪个 reference、何时读取和用于什么动作 | `test-product-context-signal-quality.sh` |
...
| R09 | 评审闭环保真 | TeamCreate 协作团队、`3 视角×max10轮`、确认轮、只重提 FAIL 视角不能因降噪被删 | 保留在 `review-orchestration.md` 并由 parity 门禁验证 | `test-product-inherited-capability-parity.sh` |
