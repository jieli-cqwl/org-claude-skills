...
assert_present '仅在进入问题澄清且根问题、直接原因或用户画像未闭合时，说明静默信息收集只形成候选线索，不冻结根问题或用户画像' "$DIRECTOR_CONVERSATION_GUIDE" "director response structure"
assert_present '操作对象：\.\.\.；边界：\.\.\.；预期产物：\.\.\.。' "$DIRECTOR_CONVERSATION_GUIDE" "director response structure"
assert_present '已完成静默信息收集：本阶段只记录线索与缺口，不冻结根问题、不冻结用户画像。' "$DIRECTOR_CONVERSATION_GUIDE" "director response structure"
assert_present '用户已明确闭合根问题、直接原因或用户画像时，不输出线索状态' "$DIRECTOR_CONVERSATION_GUIDE" "director response structure"
...
assert_present '暂停在此，只等待这一条业务事实确认，不进入 PRD / UNIT / AC / 最终 JSON。' "$DIRECTOR_CONVERSATION_GUIDE" "director response structure"
...
assert_present '本期明确排除的边界；移交时把它作为锁定基线' "$DIRECTOR_SCOPE_GUIDE" "director scope downstream-audience boundary"
assert_present '移交备注不得写入最终 `risks_and_unknowns`' "$DIRECTOR_RISKS_GUIDE" "director risk final-json boundary"
assert_absent '下游执行风险' "$DIRECTOR_RISKS_GUIDE" "director risk final-json boundary"
assert_present '不得单独手改 digest' "$DIRECTOR_FINAL_GUIDE" "director digest write boundary"
assert_present 'product-director-ledger\.json' "$DIRECTOR_FINAL_GUIDE" "director ledger artifact boundary"
assert_present 'semantic_draft / business_semantics_draft / semantics_gaps' "$DIRECTOR_FINAL_GUIDE" "director business semantics final-json boundary"
