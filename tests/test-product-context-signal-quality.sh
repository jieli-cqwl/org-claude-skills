...
assert_section_present "$DIRECTOR_SKILL" "## The Process" '项目文档、源码、github' "director silent-scan inputs"
assert_section_present "$DIRECTOR_SKILL" "## The Process" '形成候选线索' "director silent-scan outputs"
...
assert_section_present "$DIRECTOR_SKILL" "## The Process" '回到 Checklist 中尚未完成的事项，继续请求用户确认基线' "director focused co-creation"
assert_section_absent "$DIRECTOR_SKILL" "## The Process" '\| Step \| Read \| Advance when \| Stop when \|' "director process table noise"
assert_section_absent "$DIRECTOR_SKILL" "## The Process" '下游理解' "director process downstream-audience noise"
...
PY
assert_section_present "$DIRECTOR_SKILL" "## HARD-GATE" '在你向用户呈现 Director baseline，并收到用户明确回复 `产品总监确认` 之前，不要把它当成已确认基线；用户确认检查点未闭合前，不得冻结基线' "director confirmed-baseline guard"
...
assert_section_present "$DIRECTOR_SKILL" "## Role Boundary" '主动形成推荐判断并推进基线闭合；用户负责补充、确认或替换真实业务事实' "director co-creation owner boundary"
assert_section_present "$DIRECTOR_SKILL" "## Role Boundary" '用户确认前只形成候选基线判断' "director pending-baseline wording"
assert_section_absent "$DIRECTOR_SKILL" "## Role Boundary" '确认前只形成候选判断' "director ambiguous pending-baseline wording"
assert_section_absent "$DIRECTOR_SKILL" "## Role Boundary" '移交给设计、测试设计、技术负责人和交付角色' "director role-boundary downstream noise"
assert_section_absent "$DIRECTOR_SKILL" "## Role Boundary" '不得反向改写' "director downstream-audience noise"
...
assert_present '子 Agent 只返回证据与缺口，不写根问题结论' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification subagent boundary"
assert_absent '不确认结论' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification ambiguous confirmation wording"
assert_present '带着推荐请求用户确认' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification explicit confirmation heading"
assert_absent '带着推荐去确认' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification ambiguous confirmation heading"
assert_present '问题澄清推荐必须包含事实状态表' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification fact table"
assert_present '对外每轮只呈现本轮依赖事实和一个待验证事实' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification fact table"
assert_present '用户已确认 / 推测 / 冲突 / 缺失' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification fact status owner"
assert_present '每轮只请求用户确认一个事实' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification single fact confirmation"
assert_absent '停下时只确认一个事实' "$DIRECTOR_PROBLEM_GUIDE" "director problem clarification ambiguous fact confirmation"
...
assert_present '每轮只请求用户确认一个 `\[\?\]`' "$DIRECTOR_BUSINESS_GUIDE" "director business semantics explicit user confirmation"
assert_absent '每轮最多确认一个' "$DIRECTOR_BUSINESS_GUIDE" "director business semantics ambiguous confirmation"
...
assert_present '输出待用户确认且不得写入基线的成功标准草案' "$DIRECTOR_SUCCESS_GUIDE" "director success unconfirmed draft wording"
assert_absent '输出未确认成功标准' "$DIRECTOR_SUCCESS_GUIDE" "director success ambiguous unconfirmed wording"
assert_present '提交用户确认的范围边界' "$DIRECTOR_SCOPE_GUIDE" "director scope explicit user confirmation"
assert_absent 'Director 可确认的范围边界' "$DIRECTOR_SCOPE_GUIDE" "director scope ambiguous confirmation"
...
assert_present '风险判断只请求用户确认 WHY 层影响、处理动作和状态' "$DIRECTOR_RISKS_GUIDE" "director risk explicit user confirmation"
assert_absent '风险判断只确认 WHY 层影响' "$DIRECTOR_RISKS_GUIDE" "director risk ambiguous confirmation"
assert_present '开始本 Phase 前必须具备用户已确认的业务事实' "$DIRECTOR_PHASE_GUIDE" "director phase entry user-confirmed facts"
assert_present '不得请求用户回复 `产品总监确认`' "$DIRECTOR_PHASE_GUIDE" "director phase timebox confirmation guard"
assert_absent '不得进入产品总监确认' "$DIRECTOR_PHASE_GUIDE" "director phase ambiguous confirmation entry"
assert_absent '不得确认；先缩范围' "$DIRECTOR_PHASE_GUIDE" "director phase ambiguous confirmation guard"
...
assert_present '收到用户明确回复 `产品总监确认` 后，对每个 Phase 运行' "$DIRECTOR_FINAL_GUIDE" "director finalization explicit confirmation"
assert_absent '^总监确认后' "$DIRECTOR_FINAL_GUIDE" "director finalization ambiguous confirmation"
assert_absent '明确 产品总监确认' "$DIRECTOR_SKILL" "director ambiguous final confirmation signal"
assert_absent '明确 产品总监确认' "$DIRECTOR_FINAL_GUIDE" "director final guide ambiguous final confirmation signal"
