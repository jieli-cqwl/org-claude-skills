...
assert_section_present "$DIRECTOR_SKILL" "## Reference Router（按步骤读取）" '风险与未知项' "director flow route"
...
assert_section_present "$DIRECTOR_SKILL" "## Reference Router（按步骤读取）" '静默信息收集' "director flow route"
...
assert_section_present "$DIRECTOR_SKILL" "## Reference Router（按步骤读取）" '项目现状、已有文档、contracts、历史需求' "director silent-scan inputs"
assert_section_present "$DIRECTOR_SKILL" "## Reference Router（按步骤读取）" '候选线索、来源和冲突点' "director silent-scan outputs"
