...
assert_absent 'split playbook|第 [12] 段|standard-chain|canonical|资源路由：Trigger:|按需读取|需要时|若需要|落盘|当前 eval|当前验证命令|等价证据引用|可委派|尽量' "$DIRECTOR_SKILL" "director runtime narrative noise"
assert_absent 'split playbook|第 [12] 段|standard-chain|canonical|资源路由：Trigger:|按需读取|需要时|若需要|落盘|当前 eval|当前验证命令|等价证据引用|可委派|尽量' "$MANAGER_SKILL" "manager runtime narrative noise"
