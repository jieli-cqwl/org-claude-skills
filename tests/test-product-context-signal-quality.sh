...
assert_absent 'review\.md（上游三方评审结果）|读取 `review\.md`|上游架构红旗|上游测试红旗|上游红旗承接|上游审查承接' "$DESIGN_SKILL" "design downstream review-detail boundary"
assert_present '只消费 `brief\.md` / PRD / UNIT / 明确写入 `待设计决策` 的承接项；不读取产品评审明细。' "$DESIGN_SKILL" "design downstream review-detail boundary"
assert_absent '^## 上游审查承接$|review\.md 的 `审查结论`|无上游审查|^\| AR-001 \||^\| TR-001 \|' "$DESIGN_TEMPLATE" "design template downstream review-detail boundary"
assert_present '^## 产品交付承接$' "$DESIGN_TEMPLATE" "design template downstream review-detail boundary"
assert_absent 'review\.md（上游三方评审结果）|参考其三视角审查结论|避免重复审查' "$TECH_LEAD_SKILL" "tech-lead downstream review-detail boundary"
assert_present '只消费已冻结的需求、设计、测试用例和待计划约束；不读取产品评审明细，也不依赖前序评审过程来缩减本阶段审查。' "$TECH_LEAD_SKILL" "tech-lead downstream review-detail boundary"
...
assert_present '^## review\.md 产物契约$' "$MANAGER_REVIEW" "manager review artifact definition"
assert_present 'review\.md 是 Manager 阶段的评审闭环证据文件' "$MANAGER_REVIEW" "manager review artifact definition"
assert_absent '^维护 `review\.md` 时' "$MANAGER_REVIEW" "manager review artifact definition"
...
assert_present 'WARN 项在 `review\.md` 中显式承接' "$MANAGER_REVIEW" "review orchestration"
