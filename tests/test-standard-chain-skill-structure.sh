...
assert_present 'D-S2.*references/d-s2-problem-clarification\.md|references/d-s2-problem-clarification\.md.*问题澄清' "$DIRECTOR"
assert_present 'D-S3.*references/d-s3-success-appetite\.md|references/d-s3-success-appetite\.md.*价值假设.*Appetite' "$DIRECTOR"
assert_present 'D-S6.*references/d-s6-phase-planning\.md|references/d-s6-phase-planning\.md.*Phase' "$DIRECTOR"
assert_present 'D-S2~D-G1.*references/conversation-guide\.md' "$DIRECTOR"
assert_present 'D-S2~D-S6 的业务口径只读取各自 D-Sx 扩展文件' "$DIRECTOR"
