...
[ ! -e "$TMP_HOME/.codex/skills/skill-harness" ] || fail "retired skill-harness should not install as a codex skill"
...
[ -f "$TMP_HOME/.codex/skills/consistency-audit/agents/openai.yaml" ] || fail "consistency-audit should remain codex-auto"
grep -Fq 'Canonical artifact consistency and trace audit' "$TMP_HOME/.codex/skills/consistency-audit/agents/openai.yaml" || fail "consistency-audit codex adapter should expose canonical trace scope"
grep -Fq 'UNIT/AC/Task/test_ref' "$TMP_HOME/.codex/skills/consistency-audit/agents/openai.yaml" || fail "consistency-audit codex adapter prompt should mention trace object types"
grep -Fq 'Use after implementation-router returns decision=serial to execute small-chain implementation via subagents.' "$TMP_HOME/.codex/skills/subagent-driven-development/SKILL.md" || fail "subagent-driven-development codex description should require router serial decision"
grep -Fq 'description: "Use after implementation-router returns decision=serial when small-chain work needs an isolated git worktree."' "$TMP_HOME/.codex/skills/using-git-worktrees/SKILL.md" || fail "using-git-worktrees codex description should require router serial decision"
grep -Fq 'description: "Use after brainstorming produces design.md to create tasks.md, plan.md, and routing input."' "$TMP_HOME/.codex/skills/writing-plans/SKILL.md" || fail "writing-plans codex description should mention routing input"
[ -f "$TMP_HOME/.codex/skills/research/agents/openai.yaml" ] || fail "research should remain codex-auto"
grep -Fq 'Research analysis, decisions, and community discovery' "$TMP_HOME/.codex/skills/research/agents/openai.yaml" || fail "research codex adapter should expose community discovery scope"
grep -Fq 'skills, MCPs, plugins, packages, or repos' "$TMP_HOME/.codex/skills/research/agents/openai.yaml" || fail "research codex adapter prompt should mention community object types"
[ -f "$TMP_HOME/.codex/skills/scan/agents/openai.yaml" ] || fail "scan should remain codex-auto"
grep -Fq 'performance' "$TMP_HOME/.codex/skills/scan/agents/openai.yaml" || fail "scan codex adapter should expose performance scope"
