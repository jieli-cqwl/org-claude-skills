...
for skill in "${official_skills[@]}"; do
  runtime_skill="$TMP_HOME/.codex/skills/$skill/SKILL.md"
  ...
  [ ! -f "$TMP_HOME/.codex/skills/$skill/agents/openai.yaml" ] || fail "Superpowers Codex adapter should not exist: $skill"
...
for retired in verify-change archive parallel-subagent-development; do
  [ ! -e "$TMP_HOME/.codex/skills/$retired" ] || fail "retired Superpowers local skill should not install into Codex: $retired"
done
