...
assert_present 'panniantong_agent_reach' "$skill_file"
assert_present 'skills_sh_softaworks_mermaid_diagrams' "$skill_file"
assert_present 'skills_sh_github_prompt_optimizer' "$skill_file"
assert_absent 'persona_colleague_skill|persona_nuwa_skill|persona_yourself_skill|persona_midas_skill' "$skill_file"
assert_present 'bash install.sh --target all --check quick' "$skill_file"
assert_present 'Quick install gate' "$skill_file"
