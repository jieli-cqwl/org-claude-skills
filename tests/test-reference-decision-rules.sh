...
assert_present '本规范用于指导 AI Coding Agent 在代码变更过程中选择测试能力、形成验证标准，并在完成前用可复验证据支撑结论' "$TESTING"
assert_present '新增或变更的可观察行为，应先形成会因目标缺失而失败的自动化测试' "$TESTING"
assert_present '暂不适合自动化时，应说明原因，形成可复验的验收用例，并记录输入、步骤、预期结果和验证方式' "$TESTING"
assert_present '写测试或实现新行为时，应先看到目标测试因行为缺失失败，再写最小实现使其通过' "$TESTING"
assert_present '项目已有覆盖率基线时，本次变更影响范围的覆盖率应保持或提升' "$TESTING"
assert_present '新增或变更的业务逻辑、分支判断和错误处理路径，行覆盖率与分支覆盖率目标为 90% 以上' "$TESTING"
assert_present '单元测试应可独立运行并稳定复现，失败信息应定位到具体规则、分支或错误结果' "$TESTING"
assert_present '成功标准依赖数据库落库、文件读写、HTTP 调用、缓存读写、消息投递、鉴权结果、模块协作或前后端接口契约时' "$TESTING"
assert_present 'Mock、Stub、Fake 用于单元测试中的依赖隔离和异常路径构造；依赖真实协作的成功标准，应由真实依赖、测试环境或已验证集成路径支撑' "$TESTING"
assert_present '录制回放应来自首次真实响应，并记录重录周期、最近一次重录时间和格式校验结果' "$TESTING"
assert_present '当成功标准跨越用户入口、服务处理、持久化或外部依赖中的两个及以上环节时' "$TESTING"
assert_present '变更影响高频请求、批处理、数据库查询、缓存策略、并发控制、队列消费、启动耗时或资源占用时' "$TESTING"
assert_present '指标退化时，应说明原因、影响范围和用户裁决结果' "$TESTING"
assert_present '回归范围应来自影响范围分析，覆盖直接变更点、依赖链和影响面中标记需回归的对象' "$TESTING"
assert_present '变更涉及鉴权、权限、输入处理、文件上传、外部 URL、密钥、敏感数据、SQL/命令执行或跨租户数据时' "$TESTING"
assert_present '完成证据使用保留并稳定通过的测试结果；skip、xfail、flaky 结果、注释或删除的测试、无意义断言和复制实现逻辑的测试只作为风险信号，不作为通过证据' "$TESTING"
assert_present '无法证明的项目应记录阻塞、风险或用户裁决结果' "$TESTING"
assert_present '覆盖率报告无法生成时，完成结论应说明工具缺口、已覆盖的测试义务和仍未量化的风险' "$TESTING"
assert_present '新增或变更的业务逻辑、分支判断和错误处理路径覆盖率达成目标' "$TESTING"
