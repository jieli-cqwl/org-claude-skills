...
assert_present '接到代码变更后，先明确测试依据，再生成测试义务，随后选择测试层级，最后把每条义务绑定到完成前重新执行的命令（fresh command）、测试结果、覆盖率报告或验证记录' "$TESTING"
...
assert_present '从本次成功标准、验收标准（AC）、影响范围分析、代码变更点、接口契约、业务不变量、历史缺陷和专项风险提取测试依据' "$TESTING"
assert_present '专项风险包括性能、安全、兼容性、可访问性、第三方依赖和数据迁移风险' "$TESTING"
assert_present '每个依据都应转成可观察目标：输入或操作、前置状态、预期结果、边界条件、失败路径、真实依赖和影响对象' "$TESTING"
assert_present '测试依据不清时，先把“做成什么样算通过”补成可观察行为、边界条件、失败路径和真实依赖' "$TESTING"
assert_present '待裁决问题未关闭前，不给出完成结论' "$TESTING"
assert_present '新增或变更可观察行为时，先形成会因目标缺失而失败的自动化测试' "$TESTING"
assert_present '仅当验证依赖一次性人工判断、外部环境不可控、缺少可调用入口或当前工具无法稳定复现时，才使用可复验的验收用例' "$TESTING"
assert_present '并记录不自动化原因、输入、步骤、预期结果和验证方式' "$TESTING"
...
assert_present '命中以下条件时，补充对应验证；未命中的专项验证不作为交付条件，未命中原因应能从影响范围分析中说明' "$TESTING"
...
assert_present '严重程度衡量问题本身的影响，优先级衡量本次交付中应多快处理' "$TESTING"
