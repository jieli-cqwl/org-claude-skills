...
assert_present "product-manager-guide" "回应包含 Director 锁定字段、Phase 边界、范围或约束事实的替换事实时，停止当前动作，报告冲突事实和建议入口；是否进入 \`/product-director\` 由用户裁决" "$PRODUCT_MANAGER_GUIDE"
assert_present "product-manager-guide" "不得写成“回退 / 回到 / 下一步执行 \`/product-director\`”" "$PRODUCT_MANAGER_GUIDE"
