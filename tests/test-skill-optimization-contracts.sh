...
assert_present "developer" "## Eval-Safe Response Contract" "$DEVELOPER"
assert_present "developer" "DEV-OPT-1 说明模式仍输出 canonical gates" "$DEVELOPER"
assert_present "developer" "DEV-OPT-2 每条 AC 的 RED/GREEN/REFACTOR 证据索引" "$DEVELOPER"
assert_present "developer" "DEV-OPT-3 developer-report.json 骨架字段" "$DEVELOPER"
assert_present "developer" "DEV-OPT-4 缺少 canonical 输入时 BLOCKED" "$DEVELOPER"
