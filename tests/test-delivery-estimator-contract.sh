...
assert_present '## 01｜决策摘要' "$SKILL_DIR/projections/delivery-estimate-template.md"
assert_present '老板版一句话' "$SKILL_DIR/projections/delivery-estimate-template.md"
assert_present '不建议承诺条件' "$SKILL_DIR/projections/delivery-estimate-template.md"
assert_present '计划基线：.*数据日期：.*计划状态：.*起始日期：' "$SKILL_DIR/projections/delivery-estimate-template.md"
assert_present '## 04｜任务排期总览' "$SKILL_DIR/projections/delivery-estimate-template.md"
assert_present '\| WBS \| 任务 \| 负责人 \| 资源 \| 开始 \| 完成 \| 工期 \| 状态 \| 关键 \| 产出 \|' "$SKILL_DIR/projections/delivery-estimate-template.md"
assert_present '## 07｜资源与 Agent 批次' "$SKILL_DIR/projections/delivery-estimate-template.md"
assert_present '中文表头|人类可读|一眼' "$SKILL"
assert_present '老板版一句话|不建议承诺条件' "$SKILL"
