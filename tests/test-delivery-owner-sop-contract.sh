...
assert_contains "tech-lead 已冻结 plan/tasks" "$SKILL"
assert_contains "调度资源" "$SKILL"
...
assert_not_contains "标准链需要落盘时" "$SKILL"
assert_not_contains "canonical: 标准链" "$SKILL"
assert_not_contains "按需读取：" "$SKILL"
assert_not_contains 'templates/*.json' "$SKILL"
assert_not_contains "需要写结构化文件时" "$SKILL"
assert_not_contains "当前 eval/对话环境" "$SKILL"
assert_not_contains "等价证据引用" "$SKILL"
assert_not_contains "可委派的读取" "$SKILL"
assert_not_contains "尽量交给" "$SKILL"
...
assert_contains "每次响应已输出状态卡" "$SKILL"
assert_contains "触发 runtime state 变化时已更新 \`delivery-state.json\`" "$SKILL"
assert_contains "新增或更新结构化 artifact 后已同步 \`artifact-registry.json\`" "$SKILL"
assert_contains "DO-S8 提交准备时已输出交付报告并写 \`signoff-package.json\`" "$SKILL"
