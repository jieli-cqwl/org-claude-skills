...
It records a compact recovery state card and redacted probe metadata. Each `UserPromptSubmit` creates a turn-bound pending record; agents write one complete schema-2 snapshot through `codex_context_continuity.py state-update --payload '<JSON>'`. The `Stop` hook blocks until that exact session and turn is `READY`, while `recover --session-id ID --turn-id ID` returns bounded evidence without promoting state. `PreCompact` and `SessionStart --source compact` preserve and restore structured recovery references; `PostCompact` stores only summary metadata. Recovery is fail-closed: incomplete or stale state cannot be treated as ready, and compact summaries are never treated as truth.
