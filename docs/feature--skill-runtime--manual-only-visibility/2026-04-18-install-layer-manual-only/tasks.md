...
- [ ] T1 Add failing assertions for install-layer manual-only visibility
  - AC: `bash tests/test-single-source-layout.sh` fails before the implementation because the low-frequency manual-only list and corresponding runtime assertions are absent, and `bash tests/test-runtime-integrity.sh` fails before the implementation because target skills still retain auto-visible runtime artifacts.
  ...
  - Depends: -
  - Complexity: moderate
- [ ] T2 Implement install.sh manual-only maintenance for low-frequency skills
  ...
  - Depends: T1
  - Complexity: moderate
- [ ] T3 Prove runtime behavior and no formatting regressions
