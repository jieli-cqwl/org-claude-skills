...
| `CC-EVAL-006` | `CC-FM-06` speculative abstraction | "Two handlers look similar; create a shared abstraction." | reuse/extract and abstraction guardrail | Reuse only if semantics, contracts, dependency direction, and verification improve. | Creates generic helper for surface similarity. | Compares semantic equivalence and dependency direction; rejects single-use abstraction. |
