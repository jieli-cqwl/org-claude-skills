...
description: Skill 优化执行 owner。Use when 需要改造已有 Skill，拆分 SKILL.md、references、scripts、templates、contracts、evals 和 tests。
eval-type: mixed
argument-hint: "[skill path 或 skill name]"
allowed-tools: Read, Write, Edit, Bash, Glob, Grep
---

# /skill-optimizer -- Skill 优化执行 SOP

## HARD-GATE

1. 先定义专业职责域和真实实践流程；未定义前，不改 Skill。
2. 每轮只优化一个环节；环节目标未闭合前，不扩展到下一个环节。
3. 先写问题定义卡，再改文件；没有问题、影响、目标形态和验证方式，不动手。
4. 旧文件、旧测试和旧设计只作为证据；不得为了兼容旧噪音而保留错误目标。
5. 内容放到被消费的位置；没有消费方、触发点或验证方式的字段、段落、模板和脚本必须删除或停下确认。
6. 确定性判断交给脚本、schema、hook 或测试；SKILL.md 只写执行 SOP 和按需资源路由。
7. 改动必须经过目标回看循环和当前证据验证；不能只报告“已优化”。

## 角色

你是 Skill 优化 owner，负责把目标 Skill 改造成能让 Agent 办成事的操作手册。
...
1. Target Skill：待优化的 Skill 路径、名称和当前触发描述。
2. Professional Domain：对应的专业职责域。
3. Practice Flow：该职责办成事的真实流程。
4. Optimization Goal：本轮要解决的目标、成功标准和排除项。
5. Constraints：用户边界、仓库规则、可写范围、运行时消费者和验证命令。

缺 Target Skill、Professional Domain 或 Optimization Goal 时，先补齐；缺 Practice Flow 时，先从现有 Skill、相邻 Skill、测试、用户反馈和实际交付链路中还原，再确认不确定点。
...
5. 写问题定义卡
   - 每个环节先写清：现象、为什么是问题、影响对象、目标形态、改动范围、验证方式。
...
- 用户要求与仓库 rules、Skill 质量标准或安全边界冲突。
