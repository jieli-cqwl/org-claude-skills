#!/usr/bin/env python3
"""Validate the skill-auditor script execution manifest."""
...
VERIFY_COMMAND_PREFIX = "bash tests/test-skill-auditor-"
...
def validate_verification_command(command: str, label: str) -> None:
    """Constrain proof commands to the skill-auditor test harness."""
    ...
    if not command.startswith(VERIFY_COMMAND_PREFIX) or not command.endswith(VERIFY_COMMAND_SUFFIX):
        fail(f"{label} verification_command must use skill-auditor bash tests")
