...
运行时权限由 frontmatter `allowed-tools`、`HARD-GATE`、`前置条件`、`运行边界` 与流程步骤限制共同表达；不得新增单独运行时权限板块。
...
- 工具权限只放 frontmatter `allowed-tools`。
...
- 每个 skill 都不保留单独运行时权限板块，且 frontmatter `allowed-tools` 存在。
- canonical 真源、派生视图限制、validator、模板和下游消费者分别落在最接近使用点的板块。
...
- 不存在单独运行时权限板块；frontmatter `allowed-tools` 必须存在。
