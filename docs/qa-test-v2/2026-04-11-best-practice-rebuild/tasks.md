...
> Scope baseline: use the approved conversation direction from 2026-04-11 as the implementation spec for this rebuild. This work is a system rewrite of the `test-design -> qa -> project-manager Phase 3` chain, not a patch pass.

## Acceptance Checklist
- [x] T1 Unify authority map, terminology, and artifact ownership
  - AC: `contracts/skill-chain.yaml`, `test-design`, `qa`, `project-manager`, and `phase3-dispatch` define one meaning for `Phase`, `UNIT`, `unit_work_dir`, `phase_dir`, `test-cases.md`, and `qa-report.md`.
...
- [x] T5 Upgrade automation gates and contract tests from format checks to decision-quality checks
  ...
  - AC: `project-manager` completion check validates `acceptance-summary.md` against `qa-report.md` issue ledger and release recommendation.
