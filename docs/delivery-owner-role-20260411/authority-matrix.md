# Delivery Owner Authority Matrix

## 目标

冻结 `tech-lead / delivery-owner / developer / qa / user` 在本链路中的权责边界，避免执行期出现 owner 漂移，并明确当前 `delivery-owner` 的真实语义是 `Delivery Owner`。
...
| `delivery-owner` | delivery kickoff、执行编排、偏差治理、动态 gate escalation、目标级收口 | 批次/优先级重排、补证据、暂停执行、replan request | scope 变化、设计变化、业务风险接受、sign-off 拒绝 | 需求定义、技术方案发明、替代 developer/qa、单方接受业务风险 |
...
- `delivery-owner` 是 execution phase delivery owner。
- `qa` 是独立质量判断 owner。
