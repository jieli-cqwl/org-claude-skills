...
本文件沉淀极客时间「Skills 技能系统」7 讲对 `skill-auditor` 的二次方法论抽象。它不是课程原文、逐字笔记或知识库搬运，而是用于后续 `design.md` 复审、实施追踪、eval 设计和证据校验的 source map。
...
- `evals/` 必需是本地 `skill-auditor` 策略，不是 C13 直接结论。
...
| `schemas/` 必需 | S/L | `skill-auditor` 局部策略 | runtime artifact 被采用 |
| `evals/` 必需 | S/O/C99 | `skill-auditor` 局部策略 | 明确 eval 消费者 |
...
14. `evals/` 对 `skill-auditor` 必需是课程结论、官方工具约束还是本地试点？
