...
| `design.md` | skill-auditor 的最终设计裁决 | 计划与实现的上游真源 |

## 背景

本设计围绕 Harness Engineering 建立 `skill-auditor`。目标不是再写一份 Skill 写作总结，而是把 Skill 的触发、加载、引用、权限、执行、验证和演化变成可审计、可验证、可迁移的工程合同。

极客时间「Skills 技能系统」7 讲是重要信息来源，提供 Skill 结构、触发、渐进式披露、SubAgent 配合和开放标准等方法论。课程信息沉淀见 `source-notes.md`。官方 `skill-creator` 负责从零创建、评估和迭代 Skill；`skill-auditor` 负责优化已有 Skill 或草稿 Skill 的质量、稳定性和工程可验证性。

## 总裁决

`skill-auditor` 保留 Harness Engineering 方向。JSON runtime artifact、schema、semantic validator、eval 和派生 Markdown/HTML 视图是最终交付的一部分，不退回 Markdown-only 路线。
...
1. 建立 `shared/skills/skill-auditor/`，作为优化已有 Skill 或草稿 Skill 的默认入口。
2. 让 `skill-auditor` 读取目标 Skill 后，输出带证据的结构化审计、优化计划和验证结果。
...
`skill-auditor` 不替代官方 `skill-creator` 的从零访谈、with/without eval 编排和 description 触发优化能力。

`skill-auditor` 不把课程原文、Harness 文章或外部 review 全量沉淀为知识库。外部材料进入仓库时，只能变成 source map、设计裁决、审计维度、runtime 合同或验证证据。

`skill-auditor` 不在首个交付闭环中改写全仓标准链路，不直接替换 product/design/tech-lead/qa 等既有 Markdown 模板。跨链路推广需由 eval 和失败样本证明收益。
...
`skill-auditor` 审计目标 Skill 时，按六段运行链路组织发现：
...
目标目录为 `shared/skills/skill-auditor/`。
...
`skill-auditor` 是审计与优化 Skill 的入口，不处理通用代码 review、不接管官方 `skill-creator` 的新建流程。
...
| commit/deploy/delete | 不由 `skill-auditor` 自动执行 | 本轮显式授权 |
...
`shared/reference/Skill质量标准.md` 继续作为质量真源。`skill-auditor` 不重建一套平行评级，而是把运行链路审计映射回 D1-D7。
...
`new-skills` 不再保留为 legacy compatibility。官方 `skill-creator` 负责默认创建入口；`skill-auditor` 负责优化已有或草稿 Skill。安装器把旧运行时残留视为 retired skill，安装时归档清理。

| 源路径 | 目标策略 | 验证命令 | 回滚动作 | 退出条件 |
| --- | --- | --- | --- | --- |
| `shared/skills/new-skills/` | 删除旧 Skill 入口、adapter、references、scripts | `bash tests/test-skill-auditor-migration.sh` | 恢复本次删除补丁 | `skill-creator` 与 `skill-auditor` 分工通过安装和 adapter 测试 |
...
| eval 迁移样例 | 从 legacy routing 改为 retired entry 样例 | `bash tests/test-skill-auditor-evals.sh` | 恢复 eval case | 退役请求不再把 `new-skills` 作为目标 Skill |
...
| `new-skills` 与 `skill-creator` 抢入口 | 退役 `new-skills`，创建归 `skill-creator`，优化归 `skill-auditor` |

## 最终裁决

`skill-auditor` 是本仓库 Skill 治理的质量优化层。它的核心竞争力是用 Harness Engineering 把 Skill 的触发、加载、引用、执行、验证和演化转成稳定合同。
