...
function renderStatus(selector, label, count) {
  const text = count === 0
    ? '等待操作。'
    : `${label}反馈 ${count} 次：仅本页原型状态，无外部跳转或真实集成。`;
  getRequiredElement(selector).textContent = text;
}

function renderAuxiliaryStatuses() {
  renderStatus('[data-testid="download-status"]', '下载入口', state.downloadClicks);
  renderStatus('[data-testid="support-status"]', '客服入口', state.supportClicks);
}
...
function bindAuxiliaryEntries() {
  getRequiredElement('[data-testid="download-button"]').addEventListener('click', () => {
    ...
    renderAuxiliaryStatuses();
  });
  getRequiredElement('[data-testid="download-reset-button"]').addEventListener('click', () => {
  ...
  getRequiredElement('[data-testid="support-reset-button"]').addEventListener('click', () => {
    state.supportClicks = 0;
    renderAuxiliaryStatuses();
  });
