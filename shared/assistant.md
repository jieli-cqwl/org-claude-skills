...
- 写测试、实现新功能：先明确测试依据、测试义务和测试层级；新增可观察行为按 RED → GREEN → REFACTOR 推进；触达真实依赖、接口契约、安全、性能、页面、第三方或迁移风险时追加专项验证。 补充细则：`{{RUNTIME_HOME}}/reference/测试规范.md`。
- 新增实现前判断复用：先理解为什么做复用、什么算判断正确，并在新增实现前确认是否已有语义一致实现。 补充细则：`{{RUNTIME_HOME}}/reference/代码复用.md`。
- 声称任务完成前：先回到成功标准和影响范围定义验证面，再运行能直接证明结果的 fresh proving command 或形成可复验记录；完成结论按证据闭合范围命名，未闭合项进入阻塞、缺陷处理、风险接受或待裁决。 补充细则：`{{RUNTIME_HOME}}/reference/完成前验证.md`。
