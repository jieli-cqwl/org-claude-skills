...
中文对话。执行前必须复述理解，复述必须具体到操作对象和预期结果，AskUserQuestion 确认后再动手。简洁可执行。本地只读操作（Glob/Grep/Read）属于理解阶段，可在提问后等待确认期间并行执行，但不得在确认前产生工件或对外输出；外部请求（WebSearch/WebFetch）须在确认后执行。
