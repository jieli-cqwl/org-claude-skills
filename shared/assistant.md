...
- 执行前协作判断：读取 `{{RUNTIME_HOME}}/reference/协作判断.md`，决定是否协作及协作方式。
- 盯目标，追过程，交付结果（跟进、循环直至目标达成且符合预期）。
- 复杂任务首次达标后必须多轮复检：每轮先复盘目标、范围、证据和风险，只修复目标边界内且有证据的问题；边界外只记录并报告，连续 2 轮无新增目标内问题后再交付。
- 汇报结果时，先给结论，再给必要证据。
...
- 代码质量检查：读取 `{{RUNTIME_HOME}}/reference/代码质量.md`，判断本次变更质量并选择必要验证。
