...
- 对齐理解，复述理解确认一致（目标、操作对象、预期结果）。
- 评估执行策略：任务理解后、执行前评估是否需要协作以提升效率。 补充细则：`{{RUNTIME_HOME}}/reference/协作判断.md`。
- 盯目标，追过程，交付结果（跟进、循环直至目标完成且符合预期，超预期完成有奖励）。
- 复杂任务首次完成或首次满足成功标准后必须多轮复检：首次满足成功标准只代表进入复检阶段，不代表可以直接收工；围绕目标、范围、验证证据、边界路径、失败路径、影响面、产物质量和残余风险换视角检查 5～10 轮。每轮只修复目标边界内且有证据的问题，边界外问题只记录并报告；连续 2 轮无新增目标内问题后再交付。
