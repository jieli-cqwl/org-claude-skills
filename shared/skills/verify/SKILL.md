...
该脚本校验 `artifact-registry.json`、`plan.json`、`tasks.json`、Task scope、test_refs anchor、`test_cases[].assertion_target` / `test_cases[].expected_result`、`test_cases[].evidence_expectation` 和 `developer-report.json` 的 TDD 证据；`developer-report.runtime_status=BLOCKED` 时按 developer 输入阻断路由；失败则按脚本返回的 `failure_code / owner / reason` 阻断。
...
2. 建立 AC 证据矩阵
   ...
   - 解析 test_ref 指向的 `test_cases[].assertion_target` / `test_cases[].expected_result` 和 `test_cases[].evidence_expectation`。
...
- [ ] test_ref 已解析到 test-cases.json，且 `test_cases[].assertion_target` / `test_cases[].expected_result`、`test_cases[].evidence_expectation` 被独立核对。
