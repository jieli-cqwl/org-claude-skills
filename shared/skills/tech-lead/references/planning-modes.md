# 计划模式与不确定性分流

> 引用者：`tech-lead/SKILL.md`。目标是决定 `plan.json.planning_mode` 与任务解锁策略，以及遇到不确定性时回退到哪个阶段。

Trigger: 当 tech-lead 需要区分设计决策不确定性与实施可行性不确定性时读取。
Read: `references/planning-modes.md`
Expect: 标准实施/探索优先边界、先探后决步骤、AI 执行约束和草稿冻结边界。
Consume: `plan.json.planning_mode`、探索 Task 字段、计划修订记录和用户确认记录消费该资源。
Evidence: 探索任务含 hypothesis、success/failure signal、unlock_condition，未解锁任务不进入可执行清单。
Sync: planning schema、Task 解锁字段、plan template 或探索优先 eval 改动时同步更新。

## 1. 适用边界

标准流程进入 `/tech-lead` 后，必须产出 `plan.json / tasks.json` 并进入 active registry。简单需求由调用方在进入标准流程前选择其他流程；一旦进入本 skill，不再内部分流到其他流程。

## 2. 不确定性分流

先判断不确定性属于哪一类：

1. 设计决策不确定
   - 例：边界怎么划、接口契约选 A 还是 B、范围是否调整、质量属性排序是否变化
   - 处理：回退 `/design`
   - 原因：这类问题会改变方案本身，不应由 `/tech-lead` 或下游 AI 默默决定

2. 实施可行性不确定
   - 例：第三方接口能否连通、迁移能否回滚、性能是否达到阈值、并行边界是否真实成立
   - 处理：允许 `/tech-lead` 规划探索任务
   - 原因：这类问题属于实施层验证，可以先探再决定

## 3. 计划模式

### 标准实施

适用条件：
- 设计已收口
- 实施依赖清晰
- 风险可通过常规前置验证管理

要求：
- `plan.json / tasks.json` 直接输出给 AI 执行的完整实施任务
- 不允许残留未收口设计决策
- `plan.json.planning_mode` 固定为 `standard-chain`

### 探索优先

适用条件：
- 设计已收口，但存在实施可行性未知
- 未知结果可能决定后续任务是否能继续解锁

要求：
- `plan.json / tasks.json` 只输出当前已解锁批次
- 必须把探索任务写成可验证假设，而不是模糊调研项
- 探索任务除 `hypothesis / success_signal / failure_signal / unlock_condition` 外，还必须写清 `baseline_note` 与 `guardrail_note`
- 探索结果若改变路线、范围、风险接受度或上线策略，必须先回到用户确认，再刷新 `plan.json / tasks.json`
- `plan.json.planning_mode` 仍固定为 `standard-chain`；探索优先只改变 Task 解锁策略，不改变 canonical schema 枚举

## 4. 先探后决

这是探索优先模式的默认规则：

1. 先执行探索任务
2. 记录验证结果和限制条件
3. 判断结果是否改变后续决策
4. 若改变决策：暂停并等待用户确认
5. 若不改变决策：刷新 `plan.json / tasks.json`，解锁后续实施任务

## 5. AI 执行约束

下游 AI 不应接收到“暂定但尚未解锁”的任务。

原因：
- AI 容易把暂定路径当成已确认路径
- 错误假设会沿 Task 链扩散
- 多 Task 计划里最危险的是“计划看起来完整，但前提其实是错的”

因此，探索优先模式下：
- 当前批次之外的任务不得作为可执行项下发
- 后续任务只在刷新后的 `plan.json / tasks.json` 中出现

## 6. 草稿与冻结边界

任何中间草稿都只允许进入主 Agent 的回收记录：
- 不得直接替代 `plan.json / tasks.json` 的最终冻结内容
- 不得直接充当 `Scope Freeze`、`Task` 或 `evidence` 的证据
- 不得与其他版本混写成多版本并存的最终工件

最终 `plan.json / tasks.json` 必须来自单一冻结版本；如果 `Scope Freeze / Task / evidence` 不能回指同一冻结锚点，说明草稿没有收敛，必须回退重做，不得进入执行。
