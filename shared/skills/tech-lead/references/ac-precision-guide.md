...
把 Task AC 写到实施精度：明确输入、输出、边界条件和错误路径，使每条 AC 可直接转为 assert。
...
1. assert 翻译：能否直接写成一行 assert？不能 → 重写
2. 输入明确：入参字段、类型、约束是否完整？缺失 → 补充
3. 输出明确：状态码/响应结构/副作用是否具体？模糊 → 细化
4. 错误路径：至少 1 个错误场景有明确错误码？没有 → 补充
5. 复杂场景：是否涉及多条件/状态转换/前置条件？是 → 使用对应增强格式
6. 模糊词扫描：是否包含无法 assert 的模糊词？包含 → 定位遮盖的具体值并替换

## 模糊词处理

删掉一个词后，AC 还能写成 assert 吗？

- **能** → 修饰噪音，直接删除。
- **不能** → 它替代了一个具体值（数值、条件、映射或列表），写出那个值。

| AC 原文 | 模糊点 | 替换后 |
|---------|--------|--------|
| "正确处理用户输入" | "正确处理"遮盖了输入→输出映射 | "email 格式非法 → 400 INVALID\_EMAIL" |
| "快速响应查询请求" | "快速"遮盖了具体数值 | "P95 响应时间 < 200ms" |
| "支持多种导出格式等" | "等"遮盖了完整列表 | "支持 CSV、JSON、PDF 三种格式" |
