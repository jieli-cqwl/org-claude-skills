...
- 目标承接优先：`plan.md` 必须通过 `goal_fidelity_review` 显式说明上游目标由哪些 Task 承接，以及后续 `delivery-owner` 应回看的 `execution_basis_ref`
...
- [ ] `plan.md` 含 `goal_fidelity_review` / `## 目标闭环与执行度量`，且每个上游目标都已映射到当前 Task 与 execution basis
