...
ASSERTIONS: dict[str, list[dict[str, Any]]] = {
    "quick-advisory-no-report": [
        ...
        },
        {
            ...
            "none": [
                r"必须(?:先|立即|马上|现在)?.*(?:research-report\.md|docs/\{feature\})",
                r"artifacts_required:\s*(?:yes|是|research-report|docs)",
            ],
        },
    ],
...
def prepare_workspace() -> None:
    ...
    ignore = shutil.ignore_patterns("__pycache__", ".ruff_cache", "evals")
    shutil.copytree(SKILL_DIR, CURRENT_SKILL, ignore=ignore)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        shutil.copytree(tmp_path / "shared/skills/research", OLD_SKILL, ignore=ignore)
