...
DIMENSION_IDS = terms(
    "root_problem_quality|success_standard_quality|scope_tradeoff_quality|"
    "risk_judgment_quality|phase_value_slice_quality|"
    "handoff_consumability_quality|language_and_noise_quality"
)
...
CAUSE_TERMS = terms("because|causing|由于|因为|导致")
COST_TERMS = terms("causing|cost|miss|delay|slow|rework|成本|延迟|遗漏|返工")
TIME_TERMS = ("day", "week", "month", "window", "天", "周", "月", "周期")
TARGET_TERMS = ("zero", "reduce", "increase", "from", "to", "低于", "达到", "从")
...
def success_standard_quality(
    ...
    ledger_text = lower_text(ledger.get("confirmations"))
    text = goals.lower()
    measurable = has_number(text) and has_any(text, TARGET_TERMS)
    has_time_window = has_any(text, TIME_TERMS)
    checkpoint_preserved = has_number(ledger_text) and has_any(ledger_text, TIME_TERMS)
    evidence: list[str] = []
    issues: list[str] = []
    if measurable:
...
def language_and_noise_quality(
    ...
    return dimension(
        ...
        [no_forbidden_fields, no_noise_terms, substantive_ledger],
        evidence,
        issues,
        must_fail=not no_forbidden_fields or not substantive_ledger,
    )
...
def evaluate(
    ...
    max_score = len(DIMENSION_IDS) * MAX_DIMENSION_SCORE
    failures = [issue for item in dimensions for issue in item["issues"] if item["must_fail"]]
    if score < min_score:
