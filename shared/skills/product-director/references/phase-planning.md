...
8. 检查风险与未知项；只要它会改变 Phase 判断，停回风险与范围事实验证。
9. 检查是否写入 UNIT、AC、`scope_item_id`、字段、状态流转、设计或实现方案；发现后删除或退回产品经理同事后续细化。
...
物理 `phase-{N}/` 目录、`brief.json` 和 `phase-{N}/phase-prd.json` 只能在总监确认门收到明确 `产品总监确认` 且台账验证通过后写入。
