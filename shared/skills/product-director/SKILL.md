...
- 扫描项目现状、已有文档、contracts、历史需求和既有 `product-director-ledger.json`；复杂、跨源或技术诉求场景使用 agent teams 交叉取证，围绕包括不限于：源码、业务线索、技术约束、范围风险、反方质疑；只返回候选线索、来源和冲突点，不把候选线索写成已闭合事实。线索足以支撑第一条根问题假设后进入问题澄清。
...
- 基于已闭合基线按价值边界切 Phase，闭合入口条件、出口条件和 `iteration_timebox_days <= 14`。补充细则：`references/phase-planning.md`
