...
1. 执行拆解 — 在 TDD 循环前建立实现上下文。
   ...
   比例缩放：
   - 轻量（单文件 + 已有清晰模式 + 复杂度 S）：记录遵循的模式，直接进 TDD
   - 标准（2-3 文件）：代码探索 + 步骤规划
   - 完整（4+ 文件 或 不熟悉代码区域 或 task_type=探索）：全部 5 步 + 详细 mini-plan
   默认标准深度；降级到轻量需满足全部三个条件。不确定就提问，无回答则等待澄清后再继续。
2. TDD 循环 — 对每条 AC：
...
- [ ] 执行拆解已完成（代码探索 + 模式识别 + 步骤规划，深度与 Task 复杂度匹配）
