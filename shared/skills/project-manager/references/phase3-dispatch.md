...
收敛判定遵循 `{{RUNTIME_HOME}}/protocols/review-fix-loop-protocol.md`。每轮 Review-Fix / QA-Fix 循环后记录 FAIL 数量：
...
- 上限: 最多 10 轮；达上限后强制终止，终止输出要求遵循 `{{RUNTIME_HOME}}/protocols/review-fix-loop-protocol.md`
