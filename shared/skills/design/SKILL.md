...
2. 获取 canonical 输入路径
   - 通过 `preflight_check.sh` 获取 `brief`、`phase_prd`、`units` 和可选 `constitution`；不自行 glob 替代脚本输出。
3. 提取设计输入
...
- [ ] scoped validator 已运行并通过：`python3 tools/community/validate_standard_chain_phase.py --phase-dir "$PHASE_DIR"`。
