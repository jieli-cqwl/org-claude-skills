...
- Finalize 只追加 `review_closure`、`final_confirmation` 和验证收口；本审查判断已审设计是否保持产品语义。
...
| 判断 | 判定方式 |
|------|---------|
| 产品意图 | 设计没有隐式改变目标、范围、业务规则、流程、角色权限或验收语义。 |
| 用户可感知行为 | 异步化、最终一致性、降级、迁移过渡、错误提示或权限变化都有产品确认或明确承接。 |
| 业务边界 | 模块/服务边界贴合业务领域边界；PRD 待设计决策全部有回应。 |
| 冻结状态 | `key_decisions`、`modules`、`interface_boundary` 没有多个最终结论、草稿结论或未冻结版本。 |
| 口径一致 | 同一业务承诺在质量目标、验证、风险、回滚和交接里使用同一口径。 |
| 下游消费 | `/test-design` 和 `/tech-lead` 能按设计内容理解业务边界和用户影响。 |
