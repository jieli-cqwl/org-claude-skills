...
独立审查 design owner 自检后的设计产物（self-checked design artifact / canonical-shaped design artifact）是否可测试、可观测、可回归验证。
...
读取自检后的设计产物、Reviewed Design Digest、审查范围摘要、用户确认记录、open WARN 承接候选、`docs/{feature}/brief.json`、当前阶段的 `phase-{N}/phase-prd.json` 和 `phase-{N}/units/UNIT-*.json`。
...
| DT-2 | 接口契约可验证性 | 接口校验规则是否精确到可驱动自动化测试？错误码是否覆盖 PRD 异常场景？边界行为是否定义？关键决策是否可追溯到 `key_decisions`、同组 `option_analysis` 与 `input_analysis`？若 `interface_boundary`、`key_decisions` 或 `quality_attributes` 仍存在草稿或未冻结版本，直接 FAIL。 | 只评接口精确度与可验证性，不评接口架构合理性（DR-3 负责） |
