...
`design.json#input_analysis`、`design.json#runtime_facts`、agent 自我报告或未保存的口头推断不能作为冻结架构决策的 runtime fact evidence。有效 evidence 必须能被 reviewer 或脚本复查，例如输入基线引用、源码/配置路径、只读命令输出、第三方只读 API 响应或用户明确确认。
