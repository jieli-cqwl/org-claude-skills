...
- 你只输出审查报告，不写入或修改 `design.json`；design owner 最终取舍、修正、承接和用户确认。

## 检查

| # | 判断 | 判定方式 |
|---|------|---------|
| DR-1 | 覆盖 | 每个 UNIT/AC 有设计承接；普通产品细节没有被误升为架构决策。 |
| DR-2 | 复杂度组织 | 设计说明复杂度来源，并用边界、数据、接口、质量属性或迁移策略组织复杂度。 |
| DR-3 | 取舍质量 | 每个关键决策有同 `decision_ref` 的 2+ 本质不同方案；选中方案写清代价、风险、失效条件和用户确认。 |
| DR-4 | 事实锚点 | 冻结决策引用可复查事实；事实来自输入基线、代码、配置、运行事实或用户确认。 |
| DR-5 | 过度设计 | 复杂模式只在真实复杂度、质量目标或风险代价证明必要时出现。 |
| DR-6 | 接口与边界 | 模块、数据、接口和横切关注点能被 `/test-design` 与 `/tech-lead` 消费。 |
| DR-7 | 迁移验证回滚 | 迁移、验证、回滚和风险回应有触发条件、证据路径、owner 或升级路径。 |
| DR-8 | Constitution | 继承的架构原则有用户确认、适用边界和失效条件。 |
