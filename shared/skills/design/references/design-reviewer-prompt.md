...
DR-2 补充检查：检查设计决策是否显示 LLM 典型偏差——不必要的设计模式（当前只有 1 个实现却引入 Factory/Strategy）、防御过度（无证据的风险添加了防御代码）、过早抽象（"万一将来"驱动的接口）。偏差模式按 `{{RUNTIME_HOME}}/reference/设计原则.md` 的面向复杂度架构设计和三原则判断。

贯穿审查透镜：设计是否先识别需求复杂度，再说明当前架构如何组织这些复杂度；必要结构是否符合简单、合适、演化原则；备选方案、代价、风险、验证方式和调整条件是否清楚。
