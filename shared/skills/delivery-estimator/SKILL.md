...
5. 形成排期计划包
   ...
   - Markdown 必须是人类可读的正式排期计划：中文标题、中文表头、中文状态值优先；时间、文件路径、任务 ID、agent 名称和必要技术名词可以保留原文。
   - 首屏必须先给老板版一句话，再让业务和老板一眼看到：对外承诺建议、承诺口径、P50/P80/P95 日期、人类投入、最大并行工作流、最大 AI agents 并发、关键路径和首要风险。
   - 明细顺序固定为：决策摘要、关键指标、甘特图、任务排期总览、里程碑、关键路径与依赖、资源与 Agent 批次、环节投入与产出、风险与重估。
   - 任务排期总览要把日期、负责人、资源、状态、是否关键和产出放在同一张表里；WBS 细节服务扫读，不再让读者从多张英文表里拼结论。
   - owner/resource、人工关口和风险等级尽量中文化；不要为了中文化破坏文件名、路径、任务 ID、agent 名称或技术证据。
   - 风险章节必须写出不建议承诺条件，至少覆盖：范围或 AC 未冻结、关键路径验证失败、验证轮次超过估算假设、高影响风险没有 owner/缓冲/应对动作。
...
- [ ] Markdown 报告已使用中文表头、中文状态/风险等级、老板版一句话，并且首屏可直接读出承诺日期、投入、关键路径、首要风险和承诺口径。
...
- [ ] 已写明不建议承诺条件，避免业务把 P95 风险暴露误读为默认交付承诺。
