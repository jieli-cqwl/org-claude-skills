...
Do NOT return a provisional result when the user asks to audit, review, validate, accept, retain, install, or assess team-readiness for a Skill.
Do NOT ask the user to choose an audit mode.
...
Do NOT write a title-only Markdown summary or chat handoff. The Markdown summary and final response must carry enough detail for a separate repair window: every P0/P1 finding needs severity, title, one full `path:line` evidence citation, impact, repair target, and verification hint.
Do NOT include empty optional finding fields. For P2/P3 findings, omit `evidence_checks`, `claim_review`, and `severity_calibration` unless each field is fully populated with valid content.
...
Do NOT mark a scored Skill fit when Instruction Contract score is below 7; below 5 forces `unfit` unless the audit is `blocked` before scoring because required evidence cannot be inspected.
...
Do NOT include tool transcripts, progress logs, collapsed subagent summaries, recaps, or chat headers in the final audit artifact.
Do NOT use shell commands except the report validator or artifact classifier; collect target evidence with `Read`, `Glob`, and `Grep`.
Do NOT collect scope or evidence through shell wrappers such as `rg`, `sed`, `find`, `nl`, `git status`, or `ls`; Bash is limited to the two approved scripts.
Do NOT issue a formal `fit`, `conditional`, `unfit`, or `blocked` verdict unless a JSON report exists and the validator passes.
Do NOT assign E4 evidence unless the JSON report records executed verification output for the claim.
Do NOT put P0/P1/P2/P3 labels or readiness verdicts in a light scan; light scans are allowed only when the user explicitly asks for a quick, non-final direction check.
</HARD-GATE>
...
- Any E4 score has a matching `executed_verification` entry with command output.
