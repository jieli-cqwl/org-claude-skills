...
项目级自定义 Skills 目录下所有 `SKILL.md`，按目录中的实际文件扫描。全局 `{{SKILLS_HOME}}/` 不作为项目级扫描目标。
...
1. **跳过代码块**：` ``` ` 围栏和行内反引号内的路径不算引用
2. **忽略中文全角标点**：剥离 `，。、；：""''（）【】` 后再判断路径合法性
3. **忽略模板占位符**：`{{VAR}}`、`${VAR}`、`<PLACEHOLDER>` 不算路径
4. **批量失效先自证**：同一 Skill 命中 ≥ 10 处"引用不存在"时，必须先逐条验证前 3 条是否触发假阳性，假阳性率 ≥ 30% 则整批降级为"检测模式问题"，不计入技术债

违反上述任一条产生的 finding 判定为"假阳性"，不计入技术债。
...
scan 不直接输出最终评级。完整评级按 `{{SKILLS_HOME}}/skill-quality-audit/references/audit-dimensions.md`，结合 runtime evidence、eval、fresh command 和人工复核裁决。
