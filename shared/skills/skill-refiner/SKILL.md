...
description: 精修既有 first-party Skill 或既有 Skill 能力。Use when 需要先确认入口事实与假设边界，再逐环节共创职责边界、办事流程、消费者、10 个环节最佳实践和验证方式，并在 SR-F1 整体策略确认后给出最终操作判断，再一次性原地修改、重写、替换、拆分或新建承载；只读审计和批量自动优化不默认承接。
eval-type: mixed
...
- SR-S2 只确认入口事实、用户反馈和假设边界；不得把根因、最终成功标准、最终操作判断或环节策略写成已确认结论。
