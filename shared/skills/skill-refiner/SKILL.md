...
4. 每轮只优化一个环节；环节目标未闭合前，不扩展到下一个环节。
5. 每轮先读取对应环节标准，再写问题定义卡；没有问题、影响、目标形态和验证方式，不动手。
...
10. 确定性判断交给脚本、schema、hook 或测试；SKILL.md 只写执行 SOP 和按需资源路由。
...
3. 已读取的环节标准、sub agent 任务包和主 agent 验收结论。
4. 采纳或驳回的候选问题信号。
5. 已优化环节和每个环节的问题定义卡摘要。
6. 变更文件、消费关系和删除理由。
7. 已运行的验证命令、证据和结果。
8. 未解决阻断、风险和下一轮候选环节。
