...
4. 策略产出后置：原地修改、重写、替换、拆分或新建承载只在环节蓝图确认时进入策略矩阵。
...
- 在环节蓝图确认前输出新建、重写、替换或拆分结论。
- active runtime 入口和 manual-only 描述冲突。
- 测试或 eval 仍使用旧 Skill 名称或旧触发边界。

## 验收

触发描述能把既有 Skill 或既有能力打磨任务路由到当前 Skill；入口动作指向承载定位和基线共创，策略结论留到环节蓝图确认后输出，并有测试或引用扫描证明 active 入口一致。
