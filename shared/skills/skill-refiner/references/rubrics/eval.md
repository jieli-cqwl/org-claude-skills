...
6. 阶段门禁可测：如果 Skill 要求全环节共创，eval 必须证明单环节共创后提前改文件是失败。
7. proof 直接：验证命令能证明 eval 期望，而不是只跑无关静态检查。
...
- 没有覆盖“全环节策略冻结前无文件改动”的行为锚点。
- 测试要求保留已确认噪音。

## 验收

eval 和测试能证明当前 Skill 的主能力、偏好保真、阶段门禁和关键反触发；旧目标门禁已改成新目标门禁。
