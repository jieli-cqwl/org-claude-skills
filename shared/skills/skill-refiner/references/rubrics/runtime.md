...
Runtime 决定 Skill 是否真正可达、是否安全、是否和安装暴露一致。运行面不是主流程，但会影响触发和验收。
...
- Skill 声明不可自动触发，但安装入口暴露为可自动触发。
- allowed-tools 已控制的权限又在主流程长篇重复。
- hook 说明写在 Skill 主流程里，但没有 registry 证据。
- checker 输出被当作语义裁决，替代主 agent 验收。
