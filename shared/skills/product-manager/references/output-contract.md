...
- standard-chain lane 的评审闭环当前状态必须同时写入 `brief.json.review_conclusion / brief.json.issue_ledger` 与 `phase-prd.json.review_conclusion / phase-prd.json.issue_ledger`；legacy lane 可投影到 `review.md`，不能口头带过。
