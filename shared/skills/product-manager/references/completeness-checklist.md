...
| C8 | 术语定义 | 领域术语是否有统一定义？是否存在歧义术语？跨 UNIT 是否使用相同术语指同一业务概念、相同状态名指同一业务状态？不同 UNIT 的规则和排除项是否矛盾？ | Clear / Partial / Missing |
| C9 | 完成信号 | 最小闭环范围是否明确？上线标准是否可验证？成功信号是否包含基线、目标值/方向、观测窗口和数据来源？是否有全局验收标准（功能+流程+安全）？ | Clear / Partial / Missing |
| C10 | 风险前瞻 | 事前风险预判：最可能的失败原因是什么？是否有高风险操作清单（含前端控制、后端控制、日志要求）？ | Clear / Partial / Missing |
...
| AI-3 | Integration Context | 每个 UNIT 都说明业务模块、不可破坏行为、跨 UNIT 依赖和业务约束，下游无需猜测影响面 | Clear / Partial / Missing |
