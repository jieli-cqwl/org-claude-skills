...
- 技术证据输入按 `contracts/phase-prd.schema.json` 的合法 `domain` 和 `status` 表达适用性、承接或关闭状态；高风险域不得合并成一句泛化证明。
- 日期/占用区间、账款/财务流水、补偿/重试等无专用 domain 时，用 `domain=other` 写清业务不变量；交付前不得残留 `status=BLOCKED`。
