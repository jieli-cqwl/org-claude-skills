...
按全房通分支管理规范，引导用户选择场景、项目、分支字段、操作计划、Git 检查、执行和 push；低风险输入不反复确认，高风险动作必须确认，防止错项目、错仓库、错来源分支、错目标分支、错合并方向、误用已有分支和未确认推送。

## HARD-GATE

1. 低风险输入不逐步二次确认；用户已明确提供或选择的场景、项目和分支字段，可直接进入计划生成与检查。
2. 只有歧义、缺失、冲突或会改变执行对象的输入才追问；不要为了流程完整要求用户重复回复 `确认`。
3. 所有项目展示必须包含业务名、仓库名和主分支。
4. 目标分支只能由开发输入字段生成，或由开发选择/输入已有分支；不要替开发猜。
5. 操作计划必须由 `scripts/qft_branch_flow.py plan` 生成，并由 `scripts/qft_branch_flow.py validate` 校验通过；不要手写最终执行计划。
6. Git 本地写操作前必须展示 plan + preflight 总览，并要求用户确认执行。
7. `requires_user_confirmation=true` 的已有目标分支复用、preflight blocker 后只执行通过项目，必须在执行确认文案中显式命名。
8. push 必须在本地创建/复用/合并完成后单独确认。
9. 工作区不干净、remote 不匹配、preflight 阻塞或必要确认缺失时，阻塞对应项目；不要静默跳过。

## Workflow

| 步骤 | 输入 | 动作 | 输出 | 失败状态 |
| --- | --- | --- | --- | --- |
| 1. 场景识别 | 用户业务意图或场景编号 | 识别场景；仅在歧义时追问 | scenario | 场景缺失或多义则停留本步 |
| 2. 项目收集 | 项目编号、业务名或仓库名列表 | 解析项目并展示业务名、仓库名和主分支 | project set | 未知项目则停留本步 |
...
| 6. 执行与 push | 用户确认执行或推送 | 只对 preflight 通过且已确认项目执行本地 Git 操作；push 单独确认 | 完成、未执行、下一步 | 冲突或 Git 失败时停止并报告项目状态 |
...
固定规则交给脚本处理，向导负责收集输入、解释计划、展示检查结果和执行前风险确认。
...
用户意图已明确时直接识别场景；只在缺失或多义时展示业务语义供选择，不展示 Git 细节：
...
用户已给出项目、业务名或仓库名时直接解析；缺失时按短卡片展示，可多选：
...
解析项目后继续收集缺失分支字段；若存在未知项目、业务名和仓库名冲突，先展示差异并追问。
