...
把产品意图和架构设计转成可断言的测试条件，再把测试条件转成可执行测试用例。不要补写产品或架构；发现无法测试的部分，转成 typed gap。
...
好的 example 必须具体到输入、操作、状态和期望结果。只写“验证导出成功”不够；应写清导出角色、筛选条件、字段、权限、数据规模和期望证据。
...
非阻断优化建议不要写成 gap。gap 必须能指向 owner 和 next_action。
