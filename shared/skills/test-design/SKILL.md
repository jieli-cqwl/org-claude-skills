...
- review payload 仍含评审后字段、结构不等同最终 reviewed content、或 digest 无法计算时，回到 TD-S4~TD-S7 修正。
...
- 当用户询问如何发起或合并 review 时，必须先说明：只有 owner 自检通过后才能用 `reviewed_test_cases_digest` 绑定已生成 review payload 并送审；未生成 review payload、未自检通过或 digest 未生成时，不得进入 TD-S8。
...
- [ ] TD-S7.5 Generate Review Payload 已完成，`reviewed_test_cases_digest` 已由不含评审后字段的 review payload 计算生成
