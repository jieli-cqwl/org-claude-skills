...
```mermaid
graph TD
  A["TD-S1 preflight"] -->|PASS| B["TD-S2/TD-S3 测试条件"]
  A -->|BLOCKED| U["等待用户裁决"]
  B --> C["TD-S4/TD-S5 测试义务与分层"]
  C --> E["TD-S6 专项最小展开"]
  E --> F["TD-S7 typed gap 裁决"]
  F -->|blocking=true| H["停止 handoff"]
  F -->|无阻断| R["TD-S8 三视角 review"]
  R -->|FAIL| C
  R -->|PASS/WARN 已承接| G["TD-G1 写入并等待 hooks gate"]
  G -->|PASS| T["交给 /tech-lead"]
  G -->|FAIL| C
```
