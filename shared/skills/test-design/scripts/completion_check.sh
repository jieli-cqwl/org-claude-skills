...
validate_test_cases() {
    ...
    if ! jq -e '
        ...
        and all(.qa_handoff_contract[]; (.test_obligation // "" | type == "string" and length > 0)
            and (.obligation_id // "" | type == "string" and length > 0)
        ...
        and all(.cross_unit_obligations[]?; (.journey_id // "" | type == "string" and length > 0)
            ...
            and (.handoff_obligation_refs | type == "array" and length > 0)
            and (.composition_status // "" | IN("COMPOSABLE", "BLOCKED_GAP")))
        and (.special_test_triggers | type == "array")
        and all(.special_test_triggers[]?; (.trigger_id // "" | type == "string" and length > 0)
            and ((.trigger_type // "") | IN("quality_attribute", "data_architecture", "cross_cutting_concern"))
            and (.source_ref // "" | type == "string" and length > 0)
            and (.condition // "" | type == "string" and length > 0)
            and ((.qa_stage // "") | IN("QA_A", "QA_B", "QA_C", "QA_D", "NFR"))
            and ((.handling // "") | IN("TEST_CASE", "QA_HANDOFF", "TYPED_GAP")))
        ...
        and (.review_conclusion.reviewer_verdicts | type == "array" and length >= 3)
        and (["test_quality", "product", "architecture"] - ([.review_conclusion.reviewer_verdicts[].perspective] | unique) | length == 0)
        and all(.review_conclusion.reviewer_verdicts[]; ((.perspective // "") | IN("test_quality", "product", "architecture"))
            and ((.verdict // "") | IN("PASS", "WARN"))
            and (.issue_count | type == "number")
            and (.issue_count >= 0)
