...
description: Legacy reference snapshot of the previous tech-lead skill, kept only for side-by-side review. Do not use as the active standard-chain tech-lead role.
eval-type: encoded_preference
argument-hint: "[feature-name]"
allowed-tools: Read, Write, Bash, Glob, Grep, TeamCreate
---

# /tech-lead-h -- legacy tech-lead reference snapshot

> Reference-only: this folder preserves the pre-rewrite `tech-lead` behavior for comparison review. It is not installed, not a standard-chain producer, and not the active planning owner. Use canonical `/tech-lead` for all current planning work.
