...
| 来源 | 成立条件 | 核心问题 |
| --- | --- | --- |
| 专业流程 | Skill 承载真实专业职责或工程职责的办事路径。 | 这个流程是否让任务更稳定、更少返工地完成？ |
| 稳定偏好 | Skill 承载用户确认过、会重复使用的方法论或判断标准。 | 它是否稳定执行用户偏好，而不是临场发挥？ |
| 组合价值 | Skill 同时承载专业流程和稳定偏好。 | 结果提升和偏好保真是否同时成立？ |
...
| 经验证据 | 已完成 with-skill / without-skill、old/new 对比、锚点保真或用户确认。 | `retain` 或继续 `optimize`。 |
...
- `with_avg >= 4.0` 且 `uplift >= 1.0`，并且关键失败模式改善、上下文成本可接受：可支持 `retain`。
- `uplift < 0.5`：形成 `retire` 信号，先复核样本质量、替代路径和影响范围。
- 其他情况：优先 `optimize`，聚焦低分维度、噪音、误触发或成本。
...
| 信号 | 含义 | 使用条件 |
| --- | --- | --- |
| `retain` | 当前证据支持继续保留和维护。 | 运行质量无阻断问题，经验数据或用户确认足以证明价值。 |
