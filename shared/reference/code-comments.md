...
- Database schema changes must keep business semantics traceable at the authoritative schema layer. New or modified tables, columns, enum/status values, constraints, and non-obvious indexes must explain business meaning, units, precision, currency, timezone/source, allowed values, nullable/default meaning, constraint semantics, and the invariant protected by constraints or indexes.
