...
- Database schema changes must keep business semantics traceable at the authoritative schema layer. New or modified tables, columns, enum/status values, constraints, and non-obvious indexes must explain business meaning, amount units and precision, currency, time source/timezone, allowed values, nullable/default meaning, constraint/index semantics, and the invariant protected by constraints or indexes.
